 ...
 * Run: `npx tsx scripts/prompt-tools-verify.ts`.
 *
...
/** The real built-in tool set, same source as `scripts/tool-picker-verify.ts`
...
console.log("\n[mcp] the server block is counted from the surviving tools");
...
check("counts only what survived", two.includes("**chrome-devtools-mcp** (2 tools)"), two);
check("singular reads right", mcpSummaryFor(["mcp__srv__one"]).includes("(1 tool)"));
