...
[Crynta/Terax v0.5.9](https://github.com/crynta/terax-ai). Current version 0.4.63.
...
| `snapshot.rs`     | `snapshot_watch` polls the OS screenshot folders by directory mtime once a second (Windows `FOLDERID_Screenshots`, macOS `screencapture location` or Desktop, XDG Pictures and its children) and emits `tedi:snapshot`; parks while off. `snapshot_drag` starts a native file drag (Windows `SHDoDragDrop`, else the `drag` crate), so `modules/snapshot/SnapshotPreview.tsx` drops anywhere and a drop on a terminal types the path. Clipboard-only captures are not seen.                                             |
