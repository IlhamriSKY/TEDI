...
[App.tsx](src/app/App.tsx) is the top-level coordinator (~3000 lines): it owns cross-module wiring (extension bridges, the right-panel mutual-exclusion, workspace switch/close orchestration), the global shortcut-handler map, and the AI live-context bridge. **Feature internals live in `src/modules/<area>/`, not here** - App.tsx coordinates them, it does not implement them.
