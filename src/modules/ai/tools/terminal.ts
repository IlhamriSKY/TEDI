  ...
  return {
    ...
    open_terminal: tool({
      ...
      inputSchema: z.object({
        ...
        target_tab_id: flexIntOpt({ min: 0, max: PANE_ID_MAX }).describe(
      ...
      inputSchema: z.object({
        leafIds: z
          .preprocess(
            ...
            z.array(z.number().int().min(0).max(PANE_ID_MAX)).min(2),
          )
          .describe(
            "leaf_id values (2 or more) to group, from the <env> terminals/browsers lists.",
          ),
        targetTabId: flexIntOpt({ min: 0, max: PANE_ID_MAX }).describe(
    ...
    rotate_pane: tool({
      ...
      inputSchema: z.object({
        leafId: flexIntReq({ min: 0, max: PANE_ID_MAX }).describe(
          "leaf_id of the pane to rotate, from the <env> terminals/browsers lists.",
        ),
        direction: z
          .enum(["row", "col"])
          .nullable()
    ...
    open_browser: tool({
      description:
        // Trimmed of what the system prompt's `# Browser` block already says
        // (search by URL, never open links from a terminal, do not curl the same
        // page). This tool is the ONE browser tool sent on every request - the
        // rest need an open pane and are held back until there is one - so prose
        // duplicated here is billed full time.
        "Open the in-app browser at `url` - a real native browser tab, not an iframe, so any site works: dev servers, docs, search engines, logged-in pages. This is THE tool for web browsing and search; to search, pass a search URL. REUSES your one research tab by default, navigating it instead of spawning another (`reused: true` in the result); pass `new_tab: true` only when the user asks for a separate tab. Pass `read: true` to also wait for load and return the page's rendered text in THIS SAME call, which answers a one-shot fact/price/rate lookup without a second read. Returns the pane's `leafId` for the other browser tools. The first visit to a host raises an approval card and is then automatic for the session; localhost and .test dev servers never ask.",
      inputSchema: z.object({
        url: z
          .url()
          .describe(
    ...
    control_browser: tool({
      description:
        "Drive an EXISTING in-app browser pane (from the <env> browsers list, by leaf_id): pass `url` to navigate it (a page or search URL) or `action` to go back/forward/reload, or `stop` to cancel a load that is hanging. Use this to reuse an open browser instead of spawning tabs; for a brand-new browser use open_browser. Prefer Navigate And Read when you also need the page content. back/forward/reload/stop are automatic; navigating to a new host asks once, same as Open Browser.",
      inputSchema: z.object({
        leafId: flexIntReq({ min: 0, max: PANE_ID_MAX }).describe(
          "leaf_id of the target browser, taken from the <env> browsers list.",
        ),
        url: z
          .url()
          .optional()
    ...
    navigate_and_read: tool({
      ...
      inputSchema: z.object({
        leafId: flexIntReq({ min: 0, max: PANE_ID_MAX }).describe(
          "leaf_id of the target browser, from the <env> browsers list.",
        ),
    ...
    read_browser: tool({
      ...
      inputSchema: z.object({
        leafId: flexIntReq({ min: 0, max: PANE_ID_MAX }).describe(
          "leaf_id of the browser to read, from the <env> browsers list.",
        ),
    ...
    browser_type: tool({
      ...
      inputSchema: z.object({
        leafId: flexIntReq({ min: 0, max: PANE_ID_MAX }).describe(
          "leaf_id of the browser, from the <env> browsers list.",
        ),
        index: flexIntReq({ min: 0, max: 10_000 }).describe(
          "[N] index of the field, from a prior read_browser with fields:true.",
        ),
    ...
    browser_click: tool({
      ...
      inputSchema: z.object({
        ...
        index: flexIntReq({ min: 0, max: 10_000 }).describe(
          "[N] index of the element, from a prior read_browser with fields:true.",
        ),
      }),
      needsApproval: true,
    ...
    browser_screenshot: tool({
      ...
      inputSchema: z.object({
        leafId: flexIntReq({ min: 0, max: PANE_ID_MAX }).describe(
          "leaf_id of the browser to capture, from the <env> list.",
        ),
      }),
      execute: async ({ leafId }) => {
