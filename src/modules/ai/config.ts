...
- open_preview(url): open a NEW browser pane. Returns \`leafId\` so you can immediately call navigate_and_read or read_browser. Use only when no browser is open, or the user explicitly wants a separate tab.
- read_browser(leafId): get an open page's rendered text. Prefer navigate_and_read when you need to navigate + read in one step. Reads the live JS-rendered DOM, so NEVER curl/fetch a JS site (YouTube, SPAs) for content (you get empty HTML). If incomplete the page is still loading - read again.
