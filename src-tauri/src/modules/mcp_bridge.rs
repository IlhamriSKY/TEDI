...
async fn call_webview(app: &AppHandle, name: &str, args: Vec<serde_json::Value>) -> BridgeReply {
    ...
    // one-window-one-webview wrapper, so the lookup stops matching the moment
    // anything attaches a second webview to the main window. Asking for the
    // WEBVIEW keeps working either way, which matters because
