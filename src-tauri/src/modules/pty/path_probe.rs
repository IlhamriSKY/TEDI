...
fn probe(path: String, extra_dirs: Option<Vec<String>>) -> PathProbeResult {
    ...
    for tool in KNOWN_TOOLS {
        let Some(exe) = tool.bases.iter().find_map(|b| find_exe(&dir, b)) else {
            continue;
        };
        ...
        let exe_name = exe
            ...
            .unwrap_or_default()
            .to_string();
        tools.push(ToolVersion { tool: tool.name.to_string(), version, exe: exe_name });
    }
...
fn run_version(exe: &Path, arg: &str, extra_path: Option<&std::ffi::OsString>) -> Option<String> {
    ...
    // The child has exited and version output is a few bytes, so reading the
    // drained pipes to EOF here cannot block.
