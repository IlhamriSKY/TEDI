# cmdan-shell-integration (bashrc)
#
...
if [ -z "$__CMDAN_HOOKS_LOADED" ]; then
  ...
  _cmdan_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_cmdan_urlencode "$PWD")"
    if [ -z "$__CMDAN_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __CMDAN_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_cmdan_precmd:"*) ;;
    *) PROMPT_COMMAND="_cmdan_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
