...
# zsh, so we shadow $? into `_cmdan_ret`.

{
  _cmdan_user_zdotdir="${CMDAN_USER_ZDOTDIR:-$HOME}"
  [ -f "$_cmdan_user_zdotdir/.zshrc" ] && source "$_cmdan_user_zdotdir/.zshrc"
...
if [[ -z "$__CMDAN_HOOKS_LOADED" ]]; then
  __CMDAN_HOOKS_LOADED=1
  ...
  _cmdan_precmd() {
    local _cmdan_ret=$?
    printf '\e]133;D;%s\e\\' "$_cmdan_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_cmdan_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _cmdan_precmd
    add-zsh-hook preexec _cmdan_preexec
  fi

  # cmdan_open: open file in editor tab via OSC 8888.
  # Usage: cmdan_open <file>
  cmdan_open() {
    local file="$1"

    if [[ -z "$file" ]]; then
      printf "usage: cmdan_open <file>\n" >&2
      return 1
    fi
    ...
    if [[ ! -f "$file" ]]; then
      printf "cmdan_open: not a file: %s\n" "$file" >&2
      return 1
    fi

    # Emit OSC 8888 with URL-encoded file path.
    printf '\e]8888;file=%s\e\\' "$(_cmdan_urlencode "$file")"
  }
