...
Patch release. Headline user-facing additions: a `tedi` CLI launcher (`tedi .` to open the current folder), AI-generated commit messages with one-click commit + push from the Source Control panel, and unified content zoom that scales editor + diff + terminal together. Plus a sweep of PTY observability + epoch-correctness fixes that should kill the "terminal opens black forever" reports for good. The bulk of the touched-file count is project-wide Prettier + rustfmt formatting hygiene with zero behaviour change.

### Added
- **`tedi` CLI launcher.** New `src-tauri/src/modules/cli.rs` plus `tauri-plugin-single-instance` integration: `tedi .` / `tedi <path>` opens a folder as the workspace root, and `tedi <file>` adopts the file's parent as the root and opens the file in an editor tab. `--version` / `--help` short-circuit before GUI init so terminal users get plain stdout instead of a spawned window. When TEDI is already running, a second invocation forwards argv into the live window over `tedi:open-cli-target` (no duplicate window), and the frontend drains the initial target once via `cli_initial_target` before subscribing to the live event. Wired in `src-tauri/src/lib.rs` and `src/app/App.tsx`.
- **Windows installer shim (`src-tauri/installer.nsh`).** NSIS post-install hook writes a `tedi.cmd` next to `TEDI.exe`, appends the install dir to user `PATH` (`HKCU\Environment` + `WM_SETTINGCHANGE` broadcast) so freshly-spawned shells see `tedi` without logging out. `--version` / `--help` are intercepted by the `.cmd` so terminal users see real stdout — required because the GUI binary is built with `windows_subsystem = "windows"` and has detached stdio. Pre-uninstall deletes the shim; the PATH entry is intentionally left alone to avoid mass-rewriting a per-user variable on uninstall.
...
- **Per-change line stats in `git_status`.** `GitChange` gains `added` / `removed` / `binary`, populated by a single `git diff --numstat HEAD` call (handles both `old => new` and `dir/{old => new}/file` rename forms). Untracked files counted from disk with a 512 KB cap and a NUL-byte binary sniff (`count_file_lines`) so a 50 MB log file doesn't stall the panel. Frontend types in `src/modules/scm/types.ts` mirror the new shape.
- **Content-zoom shortcuts.** New `view.zoomIn` / `view.zoomOut` / `view.zoomReset` bound to Cmd/Ctrl+= (with or without Shift), Cmd/Ctrl+-, Cmd/Ctrl+0. New `Preferences.contentZoom` (clamped 0.5×–3.0×, 0.1 step, persisted) in `src/modules/settings/store.ts`. App.tsx exposes the factor as a `--content-zoom` CSS variable for CodeMirror surfaces; `useTerminalSession.ts` multiplies it into xterm's `fontSize` directly — deliberately NOT CSS `zoom`, which breaks xterm's canvas glyph positioning at non-integer factors. Editor, diff, and terminal scale together from one control.
...
- **GitDiffPane polish.** Added/removed line-count chip (computed via `computeLineStats`), a "HEAD → Working tree" header, "New file" / "Removed" badges, status-coloured status variants per change type, and a shared `DIFF_THEME` matching `AiDiffPane`. `collapseUnchanged` removed — full file is always rendered so unchanged context is visible.
- **`Cmd/Ctrl+P` freed for future quick-open.** The tab-cycle binding moved to `Cmd/Ctrl+Alt+P` in `src/modules/shortcuts/shortcuts.ts`.

### Changed
- **PTY observability + epoch correctness.** Backend logs `pty_open invoke` *before* `SPAWN_LOCK` so wedged ConPTY spawns finally leave a trail (previously the only signal was the post-spawn log line, which never landed if the spawn hung). `retryPty` / `respawnSession` rewritten to capture `myEpoch` *after* the synchronous `ptySpawnEpoch` bump in `openPtyForSession` and drop stale results, fixing a race where a duplicate retry could clobber the newer spawn. New `STUCK_RECOVERY_MS = 8 s` watchdog forces a `retryPty` / `retrySsh` when both `lastPtyError` and `pty` stay null past the 2 s container-settle budget — observed on workspace restore with split panes.
...
- **`src-tauri/Cargo.toml`** adds `tauri-plugin-single-instance = "2"` (only new crate).
...
### Internal / Tooling
- **Project-wide Prettier sweep (~170 files).** New `.prettierrc.json` (semi, double quotes, 2-space, printWidth 100, trailing comma `all`, LF, `prettier-plugin-tailwindcss`). The vast majority of the touched-file count in this release is whitespace/import-merge/Tailwind-class-reorder churn from this sweep — no behaviour change in any of those files. Future contributors run `pnpm format` once and CI is happy.
- **`rustfmt.toml`** — edition 2021, max_width 100, tab_spaces 4, `use_field_init_shorthand`, `use_try_shorthand`, `reorder_imports`, `reorder_modules`. Applied across the Rust tree (the small whitespace-only diffs in `pty/mod.rs`, `pty/session.rs`, `ssh/mod.rs`, `ssh/session.rs`, `secrets.rs`, `preview.rs`, `fs/*.rs` are from this).
- **`.editorconfig`** — 2-space default, 4-space for `*.{rs,toml}`, tab for Makefile, LF + UTF-8 + final-newline + trim-trailing-whitespace (markdown excluded from trim so deliberate trailing spaces survive).
...
Minor bump because **Restore checkpoint** is the first user-facing "undo the agent" surface — significant new capability rather than another polish pass over the 0.1.x AI loop.

### Added
- **Restore checkpoint — "undo the agent's last turn".** A new `RestoreCheckpointButton` sits next to the most recent user message (hidden mid-stream) and rolls the session back to the pre-turn state: every file the agent mutated this turn is reverted to its original content, chat history is trimmed back to before the user prompt, and the read-cache is cleared so the next turn must re-read before editing. One checkpoint per session, opened on `sendMessage` and discarded on the next prompt (no arbitrary history travel). Mutating tools (`edit`, `multi_edit`, `write_file`, `create_directory`) and the plan-review apply path all record into the active checkpoint via `recordFileMutation`. **Files you've manually edited since the agent wrote are preserved** — restore compares the on-disk content against `writtenContent` per path and skips any divergence (created files: deleted only if untouched; created dirs: deleted only if still empty). Restore-in-progress races are guarded by a `restoringSessions` Set so a quick "Restore → Send" can't lose or interleave with a `messages = trimmed` mid-flight. Checkpoints live in-process only (not persisted across app restarts) — the feature is "undo my last command", not a history time-machine. New `fs_delete` Tauri command + `native.deletePath` bridge backs the create-file rollback path. Surfaced through `useSyncExternalStore(subscribeCheckpoints, ...)` so the button auto-appears the instant the first mutation lands and auto-hides when the checkpoint is consumed.
- **Session token-usage display.** The context indicator popover gains a *Session usage* section showing cumulative **Input** / **Output** / **Cached (prompt cache hit)** tokens with a hit-rate percentage that turns emerald when ≥50%. New `SessionUsage` type on `agentMeta.usage` accumulated from per-step `onUsage` deltas (cached count comes from `inputTokenDetails.cacheReadTokens`, 0 on providers that don't report it). Lets users see provider prompt-cache savings without external tooling. Reset on session switch / clear. The legacy `lastInputTokens` / `lastCachedTokens` per-step fields were removed from the delta — running totals belong in the store, not in each tick.
- **`read_file` paging.** `read_file` now accepts `offset` (0-based line) and `limit` (max 2000 lines, default 2000). The byte cap stays at 200KB as a final safety net, but trims to the last complete newline so the output is always parseable and `nextOffset` reflects the included line count. Lets the model walk a 10k-line file with successive `offset=2000`, `offset=4000`, … calls instead of bouncing off "file too large". The compact pass keys reads by `path#offset:limit` so two paged reads of the same path are never collapsed as redundant (only same-window re-reads or post-mutation reads get elided).
- **Provider-aware prompt-cache adapter (`lib/cache.ts`).** Extracted the Anthropic cache-breakpoint placement from `agent.ts` into a provider-keyed registry. Marks three breakpoints on Anthropic requests — system message (caches system + tool schemas), last user message (conversation prefix up to current turn), last tool-result message (rolling write that advances each tool step) — for compounding savings on long tool loops. OpenAI / xAI / DeepSeek / SumoPod / Google rely on implicit prefix caching (already satisfied: byte-stable system prompt, env block lives on the last user message). Groq / Cerebras / LM Studio have no prompt cache (gateway is stateless). Anyone adding a provider just extends one switch.
- **Subagents now flow through `generateText` with cache breakpoints.** Switched from `Experimental_Agent.generate(...)` (which hides the message list) to `generateText({ messages, tools, stopWhen })` so we can inject Anthropic cache markers on the subagent's system/user messages too. Unknown subagent model ids fall back to SumoPod's runtime-discovery shape so a freshly detected model picked from the dropdown still runs subagents. `openaiCompatibleBaseURL` is plumbed through from preferences so subagents reach the same custom endpoint the main agent uses.
- **Per-tab-type accent stripe.** The 2.5px coloured stripe on the active tab is now per-kind: **emerald** local shell, **sky** SSH, **blue** file editor, **cyan** preview, **violet** AI diff, **amber** git diff. Painted by a child `<span>` rather than `data-state=active:after:` because the primitive `TabsTrigger` already attaches its own `::after` with `group-data-horizontal/tabs:` variants that collide at equal CSS specificity — the previous setup would silently lose the stripe in multi-tab / split layouts depending on emitted CSS order. The child-element approach is bulletproof.
- **PTY spawn / no-data watchdogs.** Two new failsafes on the local-PTY codepath: (1) `withSpawnTimeout` rejects `pty_open` after **15 s** if it neither resolves nor rejects — funnels a hung spawn into the existing retry-banner path so the leaf stays recoverable with Enter, instead of presenting a forever-black pane with `pty=null` AND `lastPtyError=null`. If the underlying promise resolves later (the spawn did eventually complete) the stray PTY is closed so Rust doesn't leak. (2) `armNoDataWatchdog` fires after **5 s** of total silence post-`pty_open` — surfaces the case where ConPTY/profile init wedges after acking spawn but never emits a prompt. The orphaned PTY is closed and the leaf gets a retry banner.
- **Tools cache.** `buildTools(ctx)` is now memoised per-context via a `WeakMap` — fresh chat session = fresh ctx = fresh build, but within a session the ~12 zod schemas don't get recreated on every user turn.
- **Subagent UI: real output rendering.** `run_subagent` now renders its result properly: the **type** chip (e.g. `general-purpose`) + description label, the summary in a bordered card, and footer chips for step count + duration. Opens by default when a summary is present; auto-opens on error. Header summary improved to `type · description` when both are present.
- **Lite system-prompt heuristic.** A regex pattern (`mini | nano | flash | haiku | lite | small | tiny | gemma | gpt-oss | qwen2.5-coder | coder-{1.5,3,7}b | <N>b`) auto-picks the lite system prompt for runtime-detected models without registry entries (SumoPod auto-discovery, custom OpenAI-Compatible gateways, future models). Conservative on purpose — a false positive only loses some prompt detail; a false negative just costs a few hundred tokens.
...
- **Tab strip queue chip.** The "Queued" pill (icon + label + counter) collapses to a single clock-icon prefix and the per-item amber number is dropped — the chip row already implies the order, the count was redundant noise.
- **Diff view runs without minimap.** `GitDiffPane` passes `showMinimap: false` to the shared extensions builder — the merge view already has dual scrollbars and the unchanged-region collapser; the minimap lane would just crowd it.

### Fixed
- **Tab accent stripe flicker in multi-tab / split layouts.** A 2nd-tab open used to silently break the stripe because the primitive `TabsTrigger`'s built-in `::after` and ours fought at equal specificity — the winner depended on what other classes tailwind-merge happened to keep. Resolved by painting the stripe as a child `<span>` whose visibility is computed in JS (`e.key === activeKey`).
- **Hung `pty_open` left panes black forever.** A workspace-restore race on Windows could land the `invoke("pty_open")` promise in a never-settle state, with the leaf showing nothing and `lastPtyError=null` — the Enter-to-retry handler gates on `lastPtyError` so the pane was unrecoverable. The new 15 s spawn timeout funnels it into the retry path.
- **Compact pass dropped distinct paged reads as redundant.** When the model read the same file with different `offset`/`limit`, the older pages got elided by the "same path read later" heuristic, losing content the model still needed. Compact now keys reads by `path#offset:limit` so only same-window re-reads (or post-mutation reads) are collapsed.

### Removed
- **`AgentUsageDelta.lastInputTokens` / `lastCachedTokens`.** Per-step "last" fields are no longer surfaced from the agent loop — cumulative totals are computed in `chatStore` from per-step deltas, and the UI doesn't need both flavours.
