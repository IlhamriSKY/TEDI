...
- **`/goal` is judged by an independent check, the way Claude Code, Codex and Hermes do it.** The agent used to end a goal run by printing a magic line, which a confident model prints too early and a forgetful one never prints. After every turn a short evaluator call on the same model now reads the goal, the todo list and the end of the transcript, tool results included, and answers met, not met, or blocked. Open todos count as not met without any model call. A "not met" reason is handed to the agent as the next step, so each automatic turn goes after what is actually missing. A blocked goal, a failed turn, the Stop button, a restore and a model stuck repeating the same tool call all **pause** the run instead of pushing on, and bare `/goal` or the new play button on the goal strip resumes it with its clock intact, re-sending the turn if it failed before the model said anything. Chat mode and plan mode pause it too, since it could never finish there. The strip shows the turn count, "checking…" while the evaluator runs, and why a goal paused. `/goal pause` joins `done` and `clear`. The goal wording in the system prompt no longer changes between a running and a paused goal, so pausing does not void the prompt cache. See [goalRunner.ts](src/modules/ai/lib/goalRunner.ts), [goalJudge.ts](src/modules/ai/lib/goalJudge.ts), [GoalStrip.tsx](src/modules/ai/components/GoalStrip.tsx).
- **A turn can take 50 steps instead of 15.** At 15 an ordinary multi-file task stopped halfway, often on a tool call with nothing said. The last allowed step is now made to answer in text, so a turn that does hit the cap closes with a summary you can continue from (except on Anthropic and DeepSeek, which reject that request). See [config.ts](src/modules/ai/config.ts), [agent.ts](src/modules/ai/lib/agent.ts).
- **Sub-agents know which machine they are on.** They now get the same OS and shell line and workspace root as the main agent, so a worker on Windows stops writing POSIX commands into PowerShell. A worker also honours the tool picker's switches and plan mode, a run that fails part-way returns what it already did instead of starting over and repeating its edits, and a run cut off by its step budget reports a summary marked partial instead of its last half-sentence. See [runSubagent.ts](src/modules/ai/agents/runSubagent.ts).
- **MCP servers change without a restart.** Settings is its own window and the agent read the server list once for its whole life, so a server added, fixed or removed there reached the agent only after TEDI restarted, and a removed one kept running. It is now read every turn, an edited server restarts, and one that is switched off or removed is stopped. A server that spawns but never answers is given up on after 30 seconds and not retried for two minutes (or until you switch it off and on), instead of costing up to a minute before the first word of every turn. See [mcpClient.ts](src/modules/ai/lib/mcpClient.ts), [mcpConfig.ts](src/modules/ai/lib/mcpConfig.ts).

### Added

- **`/loop <interval> <prompt>`** sends a prompt now and again every interval, for keeping an eye on something ("/loop 10m check the CI run"). It waits its turn behind a busy agent or an open approval, is queued for its own chat and never sent to another one, stops after 48 runs, and `/loop stop` ends it. See [loop.ts](src/modules/ai/lib/loop.ts).
- **The agent knows the local date and time**, so "/schedule at 3pm" or "tomorrow at 9" is no longer a guess, and a time zone no longer drifts by hours. See [envContext.ts](src/modules/ai/lib/envContext.ts).

### Fixed

- **DeepSeek works when a request asks for sub-agents.** Asking to study, audit or explore a codebase makes TEDI force the first step into a sub-agent fan-out, and every sub-agent forces its own first tool call. DeepSeek thinks by default, and its thinking mode refuses any forced tool choice with `Thinking mode does not support this tool_choice`, so on SumoPod `deepseek-v4-flash` every such turn and every sub-agent failed at once. DeepSeek is never forced now, wherever it is served from; measured afterwards, the same request ran todos, two parallel sub-agents, TEDI's own tools and a search in one turn. See [config.ts](src/modules/ai/config.ts).
- **Stop stops.** Pressing Stop while a command ran reset the status to idle and left the command running to the end, and when the command had been approved the chat then sent the turn again and ran it a second time from the start. The running command is now killed (on macOS and Linux with everything it started), the stopped call is recorded as stopped, and nothing re-sends it. Measured on the dev build: the process is gone within two seconds and the chat stays idle. See [shell.ts](src/modules/ai/tools/shell.ts), [capture.rs](src-tauri/src/modules/shell/capture.rs), [toolHistory.ts](src/modules/ai/lib/toolHistory.ts).
- **A chat can no longer be broken for good by an interrupted tool.** A tool call left without a result, after Stop, or after typing a new message instead of answering an approval card, made every provider reject every later request in that chat with a 400, even after a restart. Such calls are now answered as interrupted before each request, and an abandoned approval card no longer holds the prompt queue and `/goal` back. See [toolHistory.ts](src/modules/ai/lib/toolHistory.ts), [AgentRunBridge.tsx](src/modules/ai/components/AgentRunBridge.tsx).
- **Images from tools no longer go to SumoPod, DeepSeek, AgentRouter or an OpenAI-compatible server as base64 text.** Those endpoints take tool results as plain text, so reading a PNG or a browser screenshot put up to megabytes of base64 into every step. The agent is told the image was left out instead. Separately, switching a tool off, a server failing to connect or an extension being disabled made its past screenshots come back as raw base64 for the rest of the session; old results now always go through their compact form. See [toolHistory.ts](src/modules/ai/lib/toolHistory.ts), [agent.ts](src/modules/ai/lib/agent.ts).
- **A spent quota says so at once.** "The usage limit has been reached", "insufficient quota" and a gateway's exhausted budget were retried three times and reported as "Failed after 3 attempts". They now fail immediately with the provider's message and, where it gives one, when the quota resets. A connection that drops before any reply, the most common SumoPod failure, is retried instead of ending the turn. See [httpProxy.ts](src/modules/ai/lib/httpProxy.ts).
- **Approving an edit cannot send it to another project.** Each approval resumed the turn from whatever terminal was active at that moment, and with the diff tab in front that could be a terminal in a different folder, so an approved relative write could land there. The turn now keeps the folder it started in, and no longer rebuilds its context block, which also stops each approval from voiding the prompt cache. A request asking to fan out to sub-agents no longer fans out again on every approval. See [transport.ts](src/modules/ai/lib/transport.ts), [agent.ts](src/modules/ai/lib/agent.ts).
- **Two gateways serving the same model no longer overwrite each other.** SumoPod and AgentRouter both list `claude-opus-5`, and whichever catalogue loaded last took the id, so an AgentRouter pick read "via SumoPod". See [config.ts](src/modules/ai/config.ts).
- **Prompts queued while the agent is busy stay in their own chat and run as commands.** The queue was shared by every chat, so a prompt queued in one chat was sent to whichever chat you opened next; it now waits for the chat it was typed in. A queued `/goal clear` or `/clear` was also sent to the model as text, so the loop it was meant to stop kept going. `/clear` also clears the todo list, `>plan` turns plan mode on instead of toggling it off and dropping the queued edits, and a bare `/schedule` shows how to use it. See [composer.tsx](src/modules/ai/lib/composer.tsx), [slashCommands.ts](src/modules/ai/lib/slashCommands.ts).
- **A schedule TEDI missed does not run days later.** A command scheduled while TEDI was closed ran 1.5 seconds after the next launch, into whichever terminal had focus. Past five minutes it is now marked missed. An absolute time also wins over a stray zero delay, which ran "at 3pm" immediately. See [engine.ts](src/modules/scheduler/lib/engine.ts), [schedule.ts](src/modules/ai/tools/schedule.ts).
- **An MCP server installed under a path with a space can be added and edited.** The command was split on spaces, so `C:/Users/IT STAFF/...` became `C:/Users/IT`, and opening Edit on a working server and pressing Save broke it. Quotes are honoured now, and an edit whose command cannot start is not saved over a working one. See [McpServersCard.tsx](src/settings/sections/components/McpServersCard.tsx).
- Context trimming no longer tells the agent that a removed file read is still below when it is not, and an extension's AI tool result is capped like every other tool's. See [compact.ts](src/modules/ai/lib/compact.ts), [extensions.ts](src/modules/ai/tools/extensions.ts).
...
- **TEDI stops running git on a timer.** While TEDI was the window in use, every Explorer and the Source Control panel ran `git status` every 2.5 seconds whether anything had changed or not: measured on a live session, around 200 short-lived processes a minute and 14 to 23% of one core, and because they shared one clock the same repository was often asked two to four times in the same millisecond. A watcher now tells the views what a batch of file changes can have moved. An untouched working tree costs no git process at all, a write into an ignored file such as a log or a build cache refreshes nothing, a new ignored folder refreshes only the dimming, and the first change after a quiet spell shows up about 250 ms after it lands instead of at the next tick. Calls that are already running are shared per repository, and the Explorer no longer pays for the `+N -M` line counts only Source Control draws. Nothing runs while TEDI is in the background, a 30 second safety check stays on, and a network drive or a folder that is not a repository yet keeps the old poll. See [watch.rs](src-tauri/src/modules/git/watch.rs), [repoWatch.ts](src/modules/scm/repoWatch.ts), [api.ts](src/modules/scm/api.ts).
- **The Process Monitor reads processes without starting a PowerShell.** An extension had no way to list processes except spawning a shell, and on Windows that meant a resident PowerShell running two CIM queries in a loop: measured on a machine with 311 processes, 122 MB and 17.8% of one core. TEDI now reads the same table itself on all three platforms, in 9 to 18 ms a sample and 1.4% of one core, with memory as the private working set Task Manager shows. Update **Process Monitor** to v0.2.0 to use it. See [procs.rs](src-tauri/src/modules/procs.rs).
- **MCP no longer needs the automation port.** Six MCP tools (keys, type_text, click, drag, screenshot and eval_js) went through the WebView's debugging port, so Install MCP switched that port on for everyone. The port keeps the window rendering while it is covered, about 117 MB of renderer memory and 4.5% of a core instead of 0.1% behind another window, and anything running as you can drive TEDI through it without a token. Those tools now reach DevTools inside TEDI over the authenticated bridge every other tool already used, limited to the six methods they send and refused while the misc pack is off. Install MCP leaves the port alone and the header dot turns amber while it is open. If you installed MCP before, turn **Automation** off once under Install MCP and restart TEDI to get the memory back. See [mcp_devtools.rs](src-tauri/src/modules/mcp_devtools.rs), [McpInstallButton.tsx](src/modules/mcpInstall/McpInstallButton.tsx).
- **A beep no longer keeps an audio process alive.** The blocking and completion beeps opened an audio context and never closed it, which keeps Chromium's audio service resident, 25 MB on a live session 40 minutes after the last sound. It now closes half a second after a beep ends. See [blockingBeep.ts](src/lib/blockingBeep.ts).
- **Code shows up while the model is still writing it.** A code block in a streaming reply was a pulsing placeholder until the whole message finished, so a long answer that was mostly code had nothing to read. It now fills in as it arrives, and a shell block cannot be run until it is complete. A code block without a language is no longer drawn as inline code. See [chat-code.tsx](src/components/ai-elements/chat-code.tsx), [markdown-code.tsx](src/components/ai-elements/markdown-code.tsx).
- **Hints in the AI chat use TEDI's own tooltips** instead of the browser's native ones. See [message.tsx](src/components/ai-elements/message.tsx).

### Added

- **One switch turns every MCP server off.** Settings > Agents > MCP Servers had only a switch per server. The card header now has a master switch: off starts no server for any turn, stops the ones already running, and leaves each server's own setting alone so switching back on restores the same set. TEDI's built-in server is not affected. See [McpServersCard.tsx](src/settings/sections/components/McpServersCard.tsx), [mcpConfig.ts](src/modules/ai/lib/mcpConfig.ts).
- **Working with a ChatGPT account inside TEDI fills the AI Usage Meter's heatmap.** The Codex grid counted only Codex CLI sessions, so turns in the built-in agent never showed. Each finished turn now records its time in `~/.tedi/chatgpt-activity.json`, times only. Update **AI Usage Meter** to v0.1.14 to see it. See [codexUsage.ts](src/modules/ai/lib/codexUsage.ts).

### Fixed

- **OpenAI and ChatGPT models show their reasoning.** Without a summary request the API returns reasoning with no readable text, so the chat drew a "Reasoned" disclosure that opened onto nothing. A chat turn now asks for a summary whenever the model will actually think, and a reasoning part that still has no text is not drawn. See [reasoning.ts](src/modules/ai/lib/reasoning.ts).
- **Approving a tool no longer sends the turn twice.** The queue and `/goal` sent their own "Continue" 52 ms after the approval had already resumed the turn, so two requests ran on one chat and every tool call was duplicated in the thread and in every request after it. They now leave a turn alone while it is running or about to resume. See [composer.tsx](src/modules/ai/lib/composer.tsx).
...
- **Sub-agents work when you are signed in with a ChatGPT account.** Every fan-out failed, and failed invisibly: `run_subagents` came back reporting each task as failed with an empty error string, so the agent had nothing to read and simply did the work inline instead. Nine out of nine failed in one captured session. The cause was that a sub-agent was the only model call in the app that did not stream, and the ChatGPT backend accepts nothing else, answering `Stream must be set to true`. Fixing that alone was not enough: the same endpoint then refused the request with `Store must be set to false`, because a sub-agent never sent the per-provider options the main chat has always sent. It now sends both, on both its model calls, and a provider error that arrives with no message is turned into a readable one instead of an empty string. The practical effect is that delegation works at all on this provider, and a broad request stops filling the main conversation with the file reads a sub-agent was supposed to absorb. See [runSubagent.ts](src/modules/ai/agents/runSubagent.ts), [subagent.ts](src/modules/ai/tools/subagent.ts).
- **Clicking a terminal in another project no longer re-prices the whole conversation.** Project memory is read from the workspace root, and the workspace root follows the active terminal, so focusing a terminal that belonged to a different folder dropped the project block out of the system prompt mid-session. The system prompt is the head of the cached prefix, so changing it invalidates every cached token behind it: measured across one long session, that single click accounted for 42% of all uncached input, about 152,000 tokens re-sent at full price. The prompt now holds the workspace it started with for the life of the session, while tools still follow you into the other project. The ChatGPT endpoint is also now known to cache prompts, which it always did; being absent from that table meant the first turn after every restart sent history in one shape and the second turn in another, voiding the prefix once per run. See [transport.ts](src/modules/ai/lib/transport.ts), [cache.ts](src/modules/ai/lib/cache.ts).
- **The AI panel stays readable when you make it narrow.** It was designed at a width it can now be dragged well below, and everything inside kept its full-width layout, so controls squeezed past each other and text wrapped one character at a time. The panel now has its own compact mode driven by its own width rather than the window's, so a wide AI panel is never made compact because some other pane is narrow: below a first threshold the least important controls give way and cards go full width, and below a second only the controls you still need to reach survive. See [globals.css](src/styles/globals.css), [AiMiniWindow.tsx](src/modules/ai/components/AiMiniWindow.tsx).
- **The AI dropdowns match the rest of the app.** The agent, model and tools pickers drew their own trigger and chevron, at a different size and weight from the Source Control menus sitting a few pixels away. They now share the one style. See [AgentSwitcher.tsx](src/modules/ai/components/AgentSwitcher.tsx), [ModelDropdown.tsx](src/modules/ai/components/ModelDropdown.tsx).

### Added

- **TEDI records its own ChatGPT plan usage, so the AI Usage Meter can show it.** That extension could only read the Codex CLI's session logs, and TEDI writes none of those, so anyone who works in the built-in agent rather than the CLI watched a number that never moved: on the machine this was found on it read "Monthly 0%, as of 19d 22h ago" while the account was really at 42%, and clicking refresh re-read the same dead file. The percentages were already coming back on every single request as response headers, so they now cost nothing to collect: TEDI writes them to `~/.tedi/chatgpt-usage.json` whenever the reading actually changes, and the meter updates itself as you work. Only numbers and the plan name are written, never the session token. Install or update **AI Usage Meter** to v0.1.13 to see it. See [codexUsage.ts](src/modules/ai/lib/codexUsage.ts).
...
- **TEDI stops working when you are not looking at it.** Every build until now shipped four Chromium flags that exist for browser automation: they turn off native occlusion detection, occluded-window backgrounding, renderer backgrounding and background timer throttling. Together they mean Chromium never learns the window is covered, so the compositor and the GPU process keep producing frames at the display's refresh rate forever, whether TEDI is focused, buried under a browser, or minimised. They now ship only when the automation port is actually configured, which is opt-in and off by default. Measured on the same Chromium build with the window fully covered by another window: 4.5% of a core before, 0.1% after. What does NOT change is the timers: `--disable-background-timer-throttling` stays on for everyone, because a hidden page normally has its timers cut to 1 Hz and xterm drains pending output on a chained `setTimeout`, so a build log running in a window you switched away from would arrive twelve milliseconds of work per second and pile up until you came back. Compositing an unattended window is the entire cost; the timers are none of it. The one thing it costs is that a browser pane screenshot taken while the window is covered now takes about 2.7s instead of 165ms, because there is no composited frame to hand back; it still succeeds, well inside the 45s deadline. See [automation.rs](src-tauri/src/modules/automation.rs).
- **An idle terminal costs nothing.** Each PTY session ran a thread that woke 125 times a second forever to ask whether the shell had produced a byte, which on the sidecar daemon was measurably all of its idle CPU. The thread now parks on a condition variable and the reader wakes it, keeping the same 8ms window that coalesces a burst into one write. The flag that says the session has ended lives inside the buffer's mutex rather than beside it, because the flusher decides whether to park while holding that lock: a flag set outside it could flip between the decision and the park, and the wake would fire into an empty wait list, leaving the thread asleep on a finished session and holding its output channel open. Two tests drive a real shell through the whole chain, one of them asserting that closing a terminal releases every thread it started. See [session.rs](src-tauri/src/modules/pty/session.rs).
- **Polls stop when the thing they are polling is not on screen.** A Source Control tab that was not the active tab kept spawning four `git.exe` every 2.5s behind an overlay; the Explorer collapsed to its header stopped its git poll but kept re-listing every expanded directory every 4s; and `git rev-parse --show-toplevel` was spawned afresh by three separate pollers to answer a question whose answer had not changed, now memoised for two seconds and swept so the memo cannot grow. Extension hot reload, a 1 Hz filesystem poll that shipped in release builds for a feature only an extension author uses, backs off to five seconds after thirty quiet ticks and snaps back the moment anything moves. See [ScmStack.tsx](src/modules/scm/ScmStack.tsx), [useFileTree.ts](src/modules/explorer/lib/useFileTree.ts), [commands.rs](src-tauri/src/modules/git/commands.rs), [autoReload.ts](src/modules/extensions/autoReload.ts).
- **A finished agent stops breathing.** The badge on a terminal running an AI CLI pulses while the CLI is working or waiting for you, and `done` is held until you focus that terminal, which can be hours. A running animation keeps the compositor producing a frame every vsync, so a board of finished agents was a frame loop per badge for as long as nobody looked. It now breathes eight times and rests on its own colour. See [aiCliStatus.ts](src/modules/terminal/lib/aiCliStatus.ts).
- **Startup does less.** The file-type icon tables were built by a reduce that spread its accumulator into a new object per entry, with a second spreading reduce nested inside it, which is quadratic and ran at module evaluation on the path to the first frame: 58.6ms became 0.59ms for provably identical tables. The dictation hook, which is instantiated by a provider wrapping the whole app shell, statically imported an AI provider that only runs after you press a microphone button; it is now loaded on use, taking 92 KB out of the eager startup graph. See [fileIcons.ts](src/modules/explorer/lib/fileIcons.ts), [useWhisperRecording.ts](src/modules/ai/hooks/useWhisperRecording.ts).
- **The editor stops rebuilding itself as you type.** CodeMirror's option block was written inline, so it was a new object on every render and the editor treated each one as a reconfiguration: the whole extension tree rebuilt and an 85-binding keymap re-parsed, per keystroke, because the dirty flag re-renders the pane. See [EditorPane.tsx](src/modules/editor/EditorPane.tsx).
- **Less data crosses the IPC boundary for the same result.** A grep hit shipped its entire matched line, which for a committed minified bundle is the whole file, while the Explorer renders 240 characters of it; hits are capped at 512 characters, above both consumers. Streamed HTTP bodies are coalesced into one message per 16ms frame rather than one per chunk, which collapses a bulk download and leaves a slow token stream as it was. The SSH read pump no longer copies every chunk twice when nothing is mirroring it, and the preview proxy no longer preallocates up to 25 MiB on an upstream's say-so. See [grep.rs](src-tauri/src/modules/fs/grep.rs), [net.rs](src-tauri/src/modules/net.rs).

### Added

- **Source Control finds every repository under the workspace.** A workspace is often one folder holding several checkouts, and the panel could only ever show the one it resolved. A strip above the change list names each repository found at or below the workspace root by its path under that root, so two folders both called `app` are told apart, and switching between them is one click. See [RepoStrip.tsx](src/modules/scm/components/RepoStrip.tsx), [commands.rs](src-tauri/src/modules/git/commands.rs).
- **Editor and diff tabs share one preview slot.** Clicking through files in the Explorer or through changed files in Source Control reuses a single italic preview tab instead of piling up a strip of them, the way VS Code does it, and the two kinds share the one slot so a diff replaces a preview editor and the reverse. A tab you pinned is never taken. See [tabHelpers.ts](src/modules/tabs/lib/tabHelpers.ts).

### Fixed

- **A remote file whose size cannot be read is no longer downloaded without a limit.** The 16 MiB ceiling on opening a file over SFTP sat inside an `if let Ok(...)` on a stat, so every way a stat can fail - no permission on the parent, a server that will not stat that path, a dangling symlink - fell through to an unbounded read. The stat still runs, because refusing in one round trip beats pulling 16 MiB first, but the ceiling is now also enforced on the bytes that actually arrive. See [sftp.rs](src-tauri/src/modules/ssh/sftp.rs).
- **`git diff --numstat` no longer takes the index lock on a 2.5s poll.** Its sibling `git status` has carried `--no-optional-locks` for exactly this reason; numstat runs twice per status refresh and did not. See [commands.rs](src-tauri/src/modules/git/commands.rs).

### Removed

- **The per-file hunk expander in the change list.** The chevron that opened a file's hunks inline under its row is gone, along with the 253-line list behind it; the diff view is where a diff is read. See [ChangeRow.tsx](src/modules/scm/components/ChangeRow.tsx).
- **`xcap` and `image`.** Declared for a macOS and Linux screen-capture path that no longer exists, with no reference anywhere in the source. Removing them takes 23 transitive crates out of the build on those platforms. See [Cargo.toml](src-tauri/Cargo.toml).

## [0.4.55] - 11-09-2026

### Changed

- **The MCP server now speaks the current stateless spec AND the session handshake at once, without breaking either.** MCP's `2026-07-28` revision is a stateless rewrite that drops the `initialize` handshake, so it cannot simply replace the model every AI CLI still speaks. TEDI is now "dual-era": a client that opens with `initialize` gets exactly the session it always did, byte for byte, and a client that opens the modern way gets the stateless path. The modern additions are `server/discover` (the required discovery call, returning supported versions, capabilities and identity), a `-32022` reply naming the versions it does support when asked for one it does not, and a `resultType` on modern results, added ONLY for modern requests so a legacy reply is unchanged. No current client uses the modern model yet, so this is forward-compatibility that costs nothing today. See [server.mjs](scripts/mcp/server.mjs), [conformance-verify.ts](scripts/mcp/conformance-verify.ts).
- **The MCP server states its capabilities and tool safety the way the spec asks.** The `initialize` reply declares `tools.listChanged: false` explicitly (a tools server carries the boolean; false is honest, the server pushes no list-changed notification) and gives its `serverInfo` a display `title`. `screenshot` and `workspace` are marked non-destructive, so a client that gates on annotations no longer treats a screen capture or a workspace switch as dangerous as `eval_js`; the read tools already carried `readOnlyHint`. See [server.mjs](scripts/mcp/server.mjs), [tools.mjs](scripts/mcp/tools.mjs).

### Added

- **Install MCP now chooses between a global and a per-project config.** The dialog gained a Global / This project toggle. Global writes each CLI's user-wide config as before; This project writes that CLI's OWN standard project file inside the folder you have open, verified against each tool's current docs: `.mcp.json` for Claude Code, `.codex/config.toml` for Codex, `.gemini/settings.json` for Gemini, `opencode.json` for opencode, `.copilot/mcp-config.json` for Copilot CLI, and `.cursor/mcp.json` for Cursor. The open folder is the focused terminal's own directory, so "This project" always means the project you are actually in, not a folder picked days ago; the dialog names it so the two can never be confused. An "Enable all found" action wires every CLI on the machine in one click. See [install.ts](src/modules/mcpInstall/install.ts), [McpInstallButton.tsx](src/modules/mcpInstall/McpInstallButton.tsx).
- **An outside AI CLI can read and add to your notes and todos over MCP.** The persistent Notes panel (the checklist and free-text notes beside the SSH menu) was reachable only by TEDI's own agent through `notes_read` / `notes_write`; now it is a `notes` tool on the MCP surface too, so a CLI driving TEDI over the stdio server can read what you have jotted against the work it is doing and add a todo or a note you asked it to remember. `read` is unattended; adding asks. Like the in-app tool it can only add and complete, never delete or overwrite - the panel is yours. It is a bridge capability, so it answers on the local socket and works on every platform, and it is deliberately NOT the agent's own per-turn plan (`todo_write`) or the schedule queue: three different lists with three different lifetimes. See [notesAutomation.ts](src/modules/notes/notesAutomation.ts), [tools.mjs](scripts/mcp/tools.mjs).

## [0.4.54] - 11-09-2026

### Added

- **Worktrees: several checkouts of one repository, each on its own branch in its own folder, so two agents can work at once without one stashing for the other.** A menu beside the branch name in Source Control lists them with their uncommitted and unpushed counts, opens one as a terminal tab, removes one, and prunes the registrations whose folder is gone. The create dialog takes the branch (new or existing, and what a new one starts from), the folder (`<repo>/.worktrees/<branch>` by default, hidden from git by a `.gitignore` of `*` that excludes itself, so there is nothing to commit), a setup command remembered per repository (a fresh worktree has no `node_modules`, `vendor` or `.env`), and an agent to start, chained after the setup into one `&&` line. Every write runs from the MAIN worktree whichever checkout the panel is pointed at: `git worktree remove` run from inside the worktree being removed half-succeeds on Windows, and a create from inside one nests the new worktree under it. A removal is refused while a terminal still sits in the folder, because Windows cannot delete it and git deregisters the worktree first, which leaves a folder nothing will clean up. A locked worktree offers no Remove at all, since git refuses to force one. See [worktrees.ts](src/modules/scm/worktrees.ts), [worktreeCreate.ts](src/modules/scm/worktreeCreate.ts), [WorktreeMenu.tsx](src/modules/scm/components/WorktreeMenu.tsx).
- **A branch another worktree holds is shown as such.** `git checkout`, `git branch -d` and `-D` all refuse a branch checked out in a second worktree, and the panel used to surface that as a raw `fatal:`. The branch list marks those rows and opens the worktree instead, and the same refusal for a worktree made in a terminal a second ago becomes a sentence saying that forcing will not help. See [BranchMenu.tsx](src/modules/scm/components/BranchMenu.tsx).
- **A pull request can be checked out into a worktree of its own**, from the list or from the review view, without moving the current checkout's HEAD. The worktree is created detached and `gh pr checkout` runs inside it, so gh keeps owning the fork-and-fetch problem, and a failed checkout removes the worktree it just made. After a merge the panel offers to remove the worktree the merged branch left behind, which is also what lets the local branch be deleted. See [prWorktree.ts](src/modules/scm/prWorktree.ts).
- **The Workspaces panel groups a workspace's rows by project.** One group per repository however many worktrees it has, because every checkout resolves to the same main worktree and that is the key. Under each group sit the worktrees not already open as a tab, and right-clicking a group or a row gives New Worktree. It is never offered on the workspace itself: one workspace routinely holds panes in two projects, so it cannot say which repository the action would mean. A worktree tab is named after its project, since the folder is named after the branch, and inside a group the row leads with its branch instead. A shell that titles itself with its own binary path no longer prints that path after every row. See [WorkspacesPanel.tsx](src/modules/workspaces/WorkspacesPanel.tsx).
- **An agent can drive worktrees over MCP.** One `worktree` tool with five actions (`list`, `add`, `remove`, `prune`, `open`) rather than five tools, since every definition is a standing cost on every request. Its handler is one bridge capability, so the stdio server and TEDI's own in-process server call the same function and answer with the same text. `cwd` defaults to the focused terminal's folder, `list` is the only action that runs without an approval card, and `add` works out for itself whether the branch has to be created. See [worktreeAutomation.ts](src/modules/scm/worktreeAutomation.ts), [tools.mjs](scripts/mcp/tools.mjs).
- **A chat has a prompt rail.** A column of marks down the right edge, one per prompt you sent: click one to jump back to it, hover for its text, and the lit mark follows what you are reading. The marks pack tighter as a session grows and the rail scrolls only once they reach their minimum height, so it holds up from a full-height sidebar down to a quarter of a four-way split, and a wheel over it scrolls the chat rather than the rail. See [promptRail.ts](src/modules/ai/lib/promptRail.ts), [AiChat.tsx](src/modules/ai/components/AiChat.tsx).
- **A chat in a pane reaches the tool picker.** The pane frame draws the title and the close button, but the tool and MCP picker and the debug-request button belong to the chat, and a paned chat had no way to reach either. See [AiMiniWindow.tsx](src/modules/ai/components/AiMiniWindow.tsx).
- **A browser pane is a webview TEDI creates, on the engine TEDI itself runs on.** Six commands (`browser_place`, `browser_close`, `browser_navigate`, `browser_zoom`, `browser_list`, `browser_cdp`) put a Tauri child webview on a pane's own rectangle and drive it. That webview is [wry](https://github.com/tauri-apps/wry), the same surface the shell is drawn with, so the browser adds no engine to the download and starts no second process: a pane's page lives exactly as long as the pane. These live in core because only Rust can call `Window::add_child`; the `tedi.browser` extension owns everything above them. A native child surface composites above everything the app paints, so `browser_place` takes bounds, holes and visibility in ONE call, and the holes are what let a menu or a dialog opened over the pane stay visible and clickable while the page underneath stays live. They must move together: a window region is fixed in the window's own coordinates and does not follow a resize. Panes are Windows only in this release, for the reason under Security. See [browser/mod.rs](src-tauri/src/modules/browser/mod.rs).
- **The DevTools Protocol is reached in-process.** `browser_cdp` goes through `ICoreWebView2::CallDevToolsProtocolMethod` on a webview the app already holds, so the whole agent surface, the accessibility snapshot, trusted input, screenshots and the canvas scaling, needs no debugging port, no WebSocket, no `--remote-allow-origins`, and leaves nothing listening on loopback for another local program to drive. Every call has a deadline, because WebView2 never completes one sent to a page blocked in `alert()`, and a call WebView2 refuses outright now reports why instead of claiming the pane went away. See [browser/cdp.rs](src-tauri/src/modules/browser/cdp.rs).
- **The Browser is offered on the MCP surface.** Its one tool with an action enum is now an extension pack in the Install MCP dialog, so an outside AI CLI can be handed a real browser for the price of a single schema. See [packs.ts](src/modules/mcpInstall/packs.ts).

### Changed

- **The AI panel steps aside while a pane shows the same chat.** The panel always renders the active session, so opening that session in a pane drew one conversation twice, sharing one composer. The pane wins, the status-bar button lights from what is actually on screen, and the AI toggle opens the panel on a new chat of its own instead of reopening onto the pane's, which read as a button that did nothing. See [tabHelpers.ts](src/modules/tabs/lib/tabHelpers.ts), [useSelectionAskAi.ts](src/app/hooks/useSelectionAskAi.ts).

### Removed

- **`dock.rs` and the `WM_PARENTNOTIFY` hook.** 543 lines that reparented another process's window into TEDI's with `SetParent`, joined the two input queues so the keyboard would follow a click across a process boundary, and cut away the title bar Chrome draws inside its own client area. Every line of it existed because the browser engine lived in another process, and nothing calls it now. The maximise-rect clamp in the same window proc stays. See [lib.rs](src-tauri/src/lib.rs).

### Fixed

- **Opening a page from a terminal's preview pill no longer leaves you looking at a blank browser.** The bridge made TWO calls: the browser tool's `open`, which puts the page in a pane, and then the extension's open-a-pane command, which mints a fresh pane every time it is called. So the page loaded in the first pane and a second, empty one opened on top of it, focused. They were one call each for a browser that could only ever have ONE pane; the moment a pane got its own identity they became two panes. With a url there is now one call, and the tool focuses whichever pane it navigated; with no url there is nothing to open but a pane, which is what the command is for. See [browserBridge.ts](src/modules/extensions/browserBridge.ts).
- **An approval card could be skipped for a tool the table never freed.** `autoApprover` waives the card for the actions a builtin tool lists in `auto`, and it carried a second, independent rule left over from when the browser was core: any `open` or `navigate` carrying a `url` to a host already trusted this session ran unattended. No builtin tool takes a `url` any more, so the branch was dead in the shape it was written for, but it was never scoped to the tool it was written for either. `pane` has an `open` action, `auto` frees only `rotate`, and JSON Schema admits properties it does not declare, so a `url` riding along on `pane {action:"open"}` would have started terminals with no card. `auto` is now the whole list and the comment says so. See [mcp.ts](src/modules/ai/tools/mcp.ts).
- **The `state` tool promised a browser's url and never returned one.** A live browser pane is an `extension-panel` leaf, whose identity was `{extensionId, panelId, state}`, and `state` is a lifecycle TONE (`connected`, `error`), not content, so there was nowhere for a url to be. An agent could see that a browser pane existed and had to spend a second call to learn which page it was on. The leaf's published `title` is reported now, which for a browser pane is the page's own title and for the SQL Explorer is its connection, and the description says that instead of promising a url. See [usePaneHandles.ts](src/app/hooks/usePaneHandles.ts), [tools.mjs](scripts/mcp/tools.mjs).
- **The browser's extensions folder is created, so an ad blocker can be found rather than typed.** A browser pane loads unpacked Chrome extensions from `browser/extensions` under the app data directory, and nothing ever created that directory: the only instructions were a path in a README, and a user typing it into a file manager could not tell a wrong guess from an extension that failed to load. A failed load is silent (wry hands WebView2 every entry and discards the result), so the two look identical. The profile beside it was always created; now both are. An extension added while TEDI runs loads after a restart: whether extensions are on is a WebView2 environment option every pane shares, and deciding it per pane made the next pane fail to open while the first was still up. See [browser/mod.rs](src-tauri/src/modules/browser/mod.rs).
- **A trailing dot walked straight past the origin guard.** `guard_url` compared the parsed host against `tauri.localhost` as a string, and the `url` crate hands `http://tauri.localhost./index.html` through with the dot still on it, which is the same host as far as DNS is concerned. The module calls that comparison the one load-bearing check in the function, and it was one keystroke from being bypassed. Trailing dots come off before the comparison now, and the tests pin it along with the uppercase spelling and a lookalike host that really is somebody else (`tauri.localhost.example.com`). See [browser/mod.rs](src-tauri/src/modules/browser/mod.rs).
- **An empty chat no longer comes back as a ghost "New chat" row.** Every new chat was written to the sessions list the moment it was created, so each one opened and never used was still in history on the next launch. A chat now reaches the list with its first message, keyed off the messages rather than the derived title so an attachments-only opener that titles itself "New chat" is still saved, or when it is renamed, and every write of the list leaves out the ones still empty. See [chatStore.ts](src/modules/ai/store/chatStore.ts).
- **Right-click Paste in the editor threw on text copied on Windows.** It put the caret at `from + text.length`, counting JS string units, while CodeMirror stores each CRLF as one position, so the caret landed past the end of the document and the paste failed with "Selection points outside of document". `replaceSelection` puts the caret where the text actually landed. See [EditorPane.tsx](src/modules/editor/EditorPane.tsx).
- **A dropdown inside a dialog scrolls with the wheel.** A modal dialog locks page scroll by cancelling every wheel event outside its own subtree, and a combobox list is portaled to the body, so a branch picker in a dialog moved only with the keyboard and read as stuck. The shared command list keeps the wheel from reaching that listener, which fixes every combobox in every dialog, and a long picker can now show its scrollbar. See [command.tsx](src/components/ui/command.tsx).
- **A `ScrollArea` with a max height scrolls instead of spilling.** Its viewport was sized `h-full`, which resolves to auto inside a parent capped only by `max-height`, so the content grew to its full height and painted over whatever sat below the box, footers included. The viewport now inherits the cap. See [scroll-area.tsx](src/components/ui/scroll-area.tsx).
- **An outside AI CLI that connects while TEDI is still starting gets the extension tools.** An extension's AI tools exist only once its `activate()` has run, and the stdio server cached the first successful answer, so a CLI that connected in those first seconds saw an enabled extension with no tools and never saw `ext_*` for the rest of its session. An extension pack that is switched on and resolves to nothing is now treated as an answer still on its way, and asked again on the next call. See [server.mjs](scripts/mcp/server.mjs).

### Security

- **A page in a browser pane can reach none of TEDI's commands, and it takes three layers to say so.** Tauri gives every webview it creates its IPC bootstrap and invoke key, and defines them so no script can take them away, so a browsed page really does hold `invoke`. Capabilities alone do not stop it: with no app permission manifest, Tauri checks an app command against them only when the request comes from a REMOTE origin, and a page can reach a local one, on Windows through any registered scheme host such as the preview proxy's `http://tedi-frame.localhost`. So every app command now refuses a browser pane by its webview label, at the one handler they all pass through. Capabilities are listed by `webviews` instead of `windows`, which is what keeps plugin commands away from a pane, since a window-scoped grant reaches every webview in that window; every window TEDI opens has a webview of the same label, so nothing about the app itself moves. And a pane's own routes into the app are cut inside WebView2 before it loads a page, which is the only thing that closes the one call Tauri answers for any page at all, the fetch that hands over large chunks of what the app streams to itself. The navigation guard also refuses every one of the app's own origins, though an iframe never passes through it. See [browser/mod.rs](src-tauri/src/modules/browser/mod.rs), [lib.rs](src-tauri/src/lib.rs), [default.json](src-tauri/capabilities/default.json).
- **Browser panes are Windows only in this release.** WKWebView and WebKitGTK offer no way to cut a page off from the app's IPC, so the host refuses to create a pane there rather than hand a page a chance at the app's streams. See [browser/mod.rs](src-tauri/src/modules/browser/mod.rs).
...
- **TEDI's own agent can open a file in the editor.** `open_file` was in the shared tool table and served only to an outside CLI, with no reason recorded for the omission - so the one agent whose job is showing you what it is talking about could read any path and put none of them in front of you. Clicking the tree only reaches a path already expanded into view, and no command id takes one, so there was no other route. Every tool that stays stdio-only now says why in one place: `keys`/`type_text`/`click`/`drag` synthesise DOM input this agent does not need, `eval_js` and `inspect logs` have no in-realm twin, `screenshot` captures the window it is already inside, and `schedule`/`ai`/`save_editor` are capabilities it already has another way. See [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts).
- **Workspaces can be acted on, not just listed.** `inspect workspaces` reported every workspace, which one was active and how each is being looked at, and then an agent asked to switch to one had to tell you to click. There is no command id for any of it, so there was no route at all. `workspace` (`switch` / `create` / `rename`) is on both transports, over the same three functions an extension already reaches. The old rule that kept this read-only was written to stop a switch happening without you asking for it, and that concern is answered by the approval card every MCP call raises - the same card `pane close all`, which is worse, has always had. See [tools.mjs](scripts/mcp/tools.mjs), [store.ts](src/modules/workspaces/store.ts).

### Changed

- **A terminal read stops shipping the decoration.** An AI CLI draws its composer border as about a hundred and sixty box-drawing characters, and that border was in the preview of every window snapshot and in the text every terminal read returned, then replayed with the conversation on every later request of the turn. A run of eight or more of the same separator now collapses to three, which says "a rule is here" just as well. Measured on one real session: a third off the window snapshot, half off a terminal read. Spaces are deliberately left alone, because they are column alignment in `ls -l` and every other table, and squeezing them would corrupt data rather than trim decoration. See [usePaneHandles.ts](src/app/hooks/usePaneHandles.ts).
- **Reading one terminal no longer costs every terminal.** The poll projection had been narrowed to a single pane and the text read had not, so each of the three callers that want one pane - reading scrollback, the tail a command returns, waiting on a pane - walked and rejoined a two-hundred-row buffer for every terminal open and then threw all but one away. With four terminals open that was four times the work per call. See [usePaneHandles.ts](src/app/hooks/usePaneHandles.ts).

### Fixed

- **The agent could not type into an AI CLI it had just started.** Writing to a terminal was refused whenever the pane was busy, and a full-screen program counts as busy - but that program is READING its input, which is the whole point of typing into one. Refusing both cases left no tool able to reach a running CLI at all, while the tool description pointed at two tools this agent is not served. Watched live, it worked around the wall by opening a third terminal, putting the text on the system clipboard, focusing the target pane and firing a paste: eight tool calls, two junk tabs and your clipboard, to send four characters. A command running on the normal screen is still refused, because it is not reading input. See [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts).
- **Opening a pane or an SSH session answers with the pane's id.** Both handed back a tab and no leaf, and every terminal tool takes a leaf - so opening a terminal cost a whole extra window snapshot to rediscover an id that had been allocated a line earlier, and opening an SSH session spent up to two seconds polling for it and could give up with "did not appear in time" on a session that was in fact open. The id was never unknown, only unreturned. See [useTabs.ts](src/modules/tabs/lib/useTabs.ts), [buildLiveContext.ts](src/app/lib/buildLiveContext.ts).
- **Starting an AI CLI from the agent lights its badge.** A launch sent straight to the PTY never passes through the terminal's own input path, so the detector that recognises Claude, Codex and the rest never fired: the pane ran the CLI and showed no badge, and its status was missing from what the agent is told about your window. It is tagged explicitly now, the same way the `+` menu's Agent picker already did. See [buildLiveContext.ts](src/app/lib/buildLiveContext.ts).
- **A command that opens a full-screen program returns when it opens.** Nothing ever comes back to a shell prompt after starting an AI CLI or vim, so the wait could only run out its clock and then report a timeout on a launch that had worked - costing another round trip to discover the program was up. It now returns the moment the pane goes full-screen and says so. Relatedly, the wait's timeout is milliseconds and `10` meaning ten seconds was the obvious misread that nothing rejected: it expired before the first look, so every launch came back empty. It is floored at a second. See [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts).

### Security

- **A private pane's SSH session was reachable by leaf id.** Every capability that addresses a pane carries its own private-leaf filter, and a structural check enforces that - but the check reads one file, and the one capability that runs a command on a REMOTE HOST is registered from another, so it was the single one without the filter. Nothing could reach it in practice, because the id of a private pane appears in no listing; the change above that returns a newly opened pane's id would have been the first thing to hand one out. The filter is in place, opening a private SSH pane withholds its id at the boundary rather than relying on each server to remember, and the check now covers leaf-addressed capabilities wherever they are registered. See [useSshLeafState.ts](src/app/hooks/useSshLeafState.ts), [driver-verify.ts](scripts/mcp/driver-verify.ts).

## [0.4.50] - 09-09-2026

### Added

- **A command can be scheduled into a terminal from outside TEDI.** The schedule engine had been reachable only by TEDI's own agent since it shipped, so an AI CLI driving the window could not queue a command, cancel one, or even SEE that three were already queued against the pane it was about to use. `schedule` (`list` / `create` / `cancel`) is in the always-on `tedi` pack, on both transports, and a create goes through the same `checkedShellCommand` guard and shell-transformer chain an immediate command does: a scheduled command runs later and unattended, so the approval card raised when it is created is the only human in the loop, and a deferred command that ran a different string from the same command run now would be its own bug. `list` is auto-approved, because an agent has to be able to look before it asks. The in-app agent deliberately gets no second copy - it already has `schedule_command` on the same engine. See [tools.mjs](scripts/mcp/tools.mjs), [bridge.ts](src/modules/scheduler/lib/bridge.ts), [server.mjs](scripts/mcp/server.mjs).
- **The Dev Environment is an MCP pack.** It runs the PHP, Node, MySQL, PostgreSQL, Redis, nginx and Apache the user's sites actually sit on, and its whole agent-facing surface was three palette commands: open the panel, start everything, stop everything. The extension now registers five AI tools of its own, and it joins API Client and Database Viewer as an extension worth advertising to a connected CLI up front, behind its own switch. See [packs.ts](src/modules/mcpInstall/packs.ts).
- **The schedule engine has a self-check.** It had none among the other seventy-seven, which is how the data loss below shipped. `scheduler-verify` runs the real engine over a stubbed store with real timers: the boot race, submit and inject firing, the fired status reaching disk, cancel disarming the timer, a past-due schedule from a previous session firing after boot, an unreadable store, and prune never touching a pending row. The control sweep gained the two halves only a running window can show: a schedule firing into a terminal by itself, and focusing a pane in a background tab. See [scheduler-verify.ts](scripts/scheduler/scheduler-verify.ts), [sweep.mjs](scripts/mcp/sweep.mjs).

### Fixed

- **A schedule created while TEDI was starting erased every schedule already saved.** `boot()` loads the store and replaces the in-memory list wholesale, so a `create()` that resolved inside that window was dropped by it - and worse, the `persist()` that create ran wrote its single row over the whole saved file, so the user's other pending schedules were gone from disk as well. The window is the store's first IPC round trip, which is exactly where an agent scheduling something during startup lands. Boot is a promise every mutator awaits now, and a failure is not cached: one unreadable read retries instead of leaving scheduling dead for the life of the window, and it still rethrows so nothing goes on to write a list that never loaded. See [engine.ts](src/modules/scheduler/lib/engine.ts).
- **Focusing a pane could not reach one in a tab you were not looking at.** `state` reports every pane in every tab, so an agent routinely picks one that is not on screen - but an inactive tab is `invisible`, and each hidden terminal is additionally `display: none`, where `.focus()` does nothing at all. So `focus_pane` answered "focus did not land" for every background pane, with nothing saying the tab was the reason, and the next keystroke went wherever focus actually was. It brings the owning tab forward first now. See [usePaneHandles.ts](src/app/hooks/usePaneHandles.ts).
- **Only terminals carry zoom controls in canvas mode.** Every window header offered `- 100% +` and answered Ctrl+wheel, but a terminal is the only leaf that rescales cleanly, through xterm's own font size. Everything else took a CSS transform on top of the canvas zoom that was already scaling it. The control and the gesture are gone for the other kinds; Ctrl+wheel is still swallowed there so it cannot browser-zoom the whole app. See [CanvasView.tsx](src/modules/panes/CanvasView.tsx).
- **A context menu no longer draws two lines where a section is empty.** A separator hides itself when it is first or last, which is not enough: a group that renders nothing between two of them left both visible with no items between. A separator that directly follows another now hides too. See [context-menu.tsx](src/components/ui/context-menu.tsx).
- **The block that animates while a turn is waiting no longer reads as a short loop.** Every other gait is a function of where the cell sits, because every other gait stands for something with an order to it. Waiting has none, and eight cells sharing one duration come back into phase every cycle however you stagger them, so within a few seconds the eye has the pattern. The `wait` gait draws a per-cell phase AND a per-cell duration once per mount, so eight cells on eight periods have no common cycle to return to. Drawn in a `useState` initialiser rather than in render, because the elapsed clock ticking beside it re-renders this component every second, and fresh numbers on each tick would restart all eight animations: a stutter rather than a shimmer. See [pixel-activity.tsx](src/components/ui/pixel-activity.tsx).

## [0.4.49] - 08-09-2026

### Added

- **Every tool TEDI can call on itself is named in the step line.** `TOOL_LABELS` covered six of the twenty tools the in-process MCP server serves, so a turn that inspected the window, opened a file, pressed a key, took a screenshot or evaluated an expression reported `Calling mcp__tedi__eval_js` at the user, which is the raw SDK key and the exact thing that map exists to avoid. All twenty are labelled now (`Inspecting extensions`, `Opening main.tsx`, `Pressing Ctrl+Shift+P`, `Capturing the window`, `SSH open`), and because the gait of the running block is read off that label, the same line of work fixes both. `step-motion-verify` imports the shared tool table both transports serve and fails if a tool is added to it without a label. See [agent.ts](src/modules/ai/lib/agent.ts), [step-motion-verify.ts](scripts/ai/step-motion-verify.ts).

### Changed

- **The block that animates while a turn runs moves differently for each thing the turn is doing.** One diagonal sweep ran for every step, so the animation was decoration: it said a turn was alive and nothing else. There are eleven gaits now, and the gait is the readout. Reading a file scans row by row like two lines of text; a grep darts about a fixed scatter; editing steps a hard-edged caret left to right and deleting runs the same caret backwards; a shell command flashes the whole strip at once at 0.55s; a fetch alternates two checkerboard fields; a subagent fan-out expands from the centre; a plan ticks row by row; and a backoff breathes the whole strip at 1.9s. A gait is a per-cell phase, a duration and which of two keyframes to run, so a new one costs a row in a table and no new CSS. See [pixel-activity.tsx](src/components/ui/pixel-activity.tsx), [stepMotion.ts](src/modules/ai/lib/stepMotion.ts), [globals.css](src/styles/globals.css).
- **A tool TEDI did not write is read off its own name.** An extension's AI tool and a third-party MCP server's tool both reach the step line as `Calling <name>` with nothing else to go on, which after a few MCP servers are connected is most of what a turn does. The name is not nothing: these are conventionally `verb_noun`, so the verb is recoverable. The `mcp__<server>__` prefix is cut, the camel hump is split, and the tokens are matched left to right against a stem table, which puts `list_issues` on the search gait, `create_pull_request` on write, `read_file` on read and `api_client_send` on the network one. Left to right, not right to left, because backwards `create_pull_request` reads as `request`; and cutting the prefix first is what stops a server named `run` deciding the gait for everything it serves. An unrecognisable name still falls back to the generic sweep, which is where all of them were. See [stepMotion.ts](src/modules/ai/lib/stepMotion.ts).
- **Nothing in the interface trails off in an ellipsis.** The trailing `...` on a menu item is a convention for "this opens a dialog" and the trailing one on a status line is a convention for "still going", and TEDI used both in about a hundred and sixty places, in two different characters. They are gone: the SSH and Explorer trees offer `Upload Files Here`, `Download Folder` and `Permissions`, the editor offers `Save As`, every search box is `Search models` rather than `Search models...`, and the `tedi` CLI prints `Verifying` and `Installing`. What stays is the ellipsis that carries information rather than decoration: the one marking text that was actually cut (a truncated path in a step line, a long grep hit), and the ones in the AI-CLI detector, which parse Claude Code's and Codex's own output and are not TEDI's to restyle. See [SshFileExplorer.tsx](src/modules/ssh/SshFileExplorer.tsx), [FileTreeNode.tsx](src/modules/explorer/FileTreeNode.tsx), [cli_update.rs](src-tauri/src/modules/cli_update.rs).

### Fixed

- **The pixel block's sweep travelled the opposite way to the one its own comments described.** A negative `animation-delay` runs a cell AHEAD, so the cell furthest along its cycle is the one that lit most recently: ordering the cells by a phase that grows toward the far corner made the head travel from that corner back to the origin, and the file had claimed left to right since it was written. The delay is now computed from the complement of the phase, so phase 0 is the cell you see light first and a gait named for a direction goes that way. See [pixel-activity.tsx](src/components/ui/pixel-activity.tsx).
- **A failing extension's status item read as a disabled one.** The `error` tone hung a small red dot off the corner of the icon and left the icon itself at the 40% muted opacity that means "off", so a broken extension looked switched off with something stuck to it. The icon now takes the blocked colour itself, which is one glyph in the colour of what it means and the way every other state in that row already works. A bitmap icon cannot be tinted, so it goes to full opacity instead of muted-and-grey. See [ExtensionStatusItems.tsx](src/modules/extensions/components/ExtensionStatusItems.tsx).

## [0.4.48] - 08-09-2026

### Added

- **An extension can open ANOTHER extension's panel.** `ctx.tabs.openExtensionTab({ extensionId })` and `openExtensionPane` take the id of whoever owns the panel, defaulting to your own, which is how an extension hands the user off to a tool that does the next step better than it could: Dev Environment knows the connection details for the database it is running, SQL Explorer knows how to browse one, and before this there was no route between them except telling the user to go and find it. It moves no data and reads nothing - the panel is rendered by the extension that owns it, exactly as if the user had opened it - so it stays on `tabs:open`, the permission for putting a tab on screen, and needs no new grant. It returns `null` when that extension is not installed, not enabled, or has no renderer for the panel, rather than opening an empty frame named after something the user may not have. Being an option field it is invisible to an older host, so it ships with `openExtensionTab.extensionId` in `HOST_FEATURES`: `ctx.has("openExtensionTab.extensionId")` is the check, and an extension using it needs no `engines.tedi` bump. See [host.ts](src/modules/extensions/host.ts), [tedi.d.ts](extensions/tedi.d.ts).

### Changed

- **The running-turn indicator is coloured by the reasoning level the turn is running at.** The pixel block that animates while the AI works was muted at every level and foil at `max`, so the deepest setting was visible and the four below it were not - the picker said `high` in amber and the thing that ran said nothing. Both indicators, the chat's status row and the status-bar pill, now take the level's own ink from the same table the picker reads, `Auto` staying muted because that is what the picker shows for it too. One table and one hook in `lib/effort.ts` rather than a colour map per surface, since a picker and an indicator that disagree about what `high` looks like are worse than neither being coloured. See [effort.ts](src/modules/ai/lib/effort.ts), [ReasoningDropdown.tsx](src/modules/ai/components/ReasoningDropdown.tsx).
- **The block is a 2x4 strip, not a 4x4 square.** Both places that draw it sit on one line of text - the status pill is 24px tall including its border, the chat's running row is a single 11.5px line - and a 22px square touched both edges of each. A strip is 10px, the height the 12px glyphs in the pill's other states occupy, so the running state finally sits on the same baseline as the rest. The diagonal chase follows the new geometry on its own: the wave now crosses four steps instead of six and reads as a head travelling left to right rather than corner to corner. The `rows`/`cols` props went with it, because both call sites want the same strip and a knob with one setting is a knob to keep in sync for nothing. See [pixel-activity.tsx](src/components/ui/pixel-activity.tsx).

### Fixed

- **The `max` foil in the pixel block was a flat cyan, not a foil.** `.tedi-foil` leaves its two repeating gradient layers at `background-size: auto`, which resolves to the element's own box - on a 4px cell that is a 4px tile, so the layer repeats every 4px and the per-cell `--fx` offset that is supposed to make eight cells sample ONE continuous sheet wrapped inside a single period instead. Every cell drew the same crop. The block sat beside a picker rendering the word `max` as a full cyan-violet-pink prism and claimed in its own comments to be cut from that same sheet. The layers are now sized to the block, so a cell is genuinely a window onto it and the hue run crosses the strip exactly as it crosses the word. See [globals.css](src/styles/globals.css), [pixel-activity.tsx](src/components/ui/pixel-activity.tsx).
- **A status-bar tooltip's calendar ran its last month label off the popover.** Month labels are absolutely positioned on the column pitch and are wider than the column they name, which is fine everywhere except the end: the last month labels one of the final columns, so `Sep` started 5px from the grid's right edge and finished flush against the popover's border, clipped mid-word during the open animation. The overhang now stops at the grid's right edge. `100%` inside a `translate` is the element's own width, so the clamp costs no guess at how wide a month name is in the user's locale. Reported on [AI Usage](https://github.com/IlhamriSKY/tedi.ai-usage) 0.1.12, whose year heatmap is the first grid wide enough to hit it. See [ExtensionStatusItems.tsx](src/modules/extensions/components/ExtensionStatusItems.tsx).
...
- **A status-bar tooltip can draw a year as a calendar grid.** `StatusItemDetailChart` gains `mode: "cells"`. The default, `"columns"`, stays a trend: one column per value, filled from the bottom, so the shape reads as a line. `"cells"` fills each column top to bottom before moving right and shades each cell by its value, which is the grid GitHub draws for contributions - `rows: 7` and a value per day is a year of activity. Both modes are one markup path, because only what a value MEANS differs: a column of cells is a bar in one and a week of days in the other. A cells grid gets 53 columns rather than 48, a full year of weeks, which the wider `detail` popover affords. First user is [AI Usage](https://github.com/IlhamriSKY/tedi.ai-usage) 0.1.12, which puts a year of Claude prompts and Codex sessions above its meters. See [registries.ts](src/modules/extensions/registries.ts), [ExtensionStatusItems.tsx](src/modules/extensions/components/ExtensionStatusItems.tsx).
- **`.tedi-glass-surface`, `.tedi-glass-solid` and `.tedi-glass-float`, so an extension panel can survive glass mode.** Glass drives `--background` toward transparent, which is what lets the editor and terminal show the wallpaper - and it means any panel painting that token dissolves, leaving its rows and trees sitting unreadable on the wallpaper. An extension cannot fix this from its own stylesheet: the floors are computed from the user's opacity slider, which only the host knows. Naming one of these classes is now the whole fix, and they are inert when glass is off. See [globals.css](src/styles/globals.css), [extensions/README.md](extensions/README.md).

### Changed

- **Every surface that draws an extension's icon resolves it the same way.** `RightPanelToggleButtons` was the one of seven that never called `resolveExtIcon`, so a `lucide:` name was simply unusable on a `surface: "right"` panel - and that gap is WHY core carried an `ICON_MAP` hard-coding a glyph for `tedi.screenshot` and `tedi.secondary-folder-tree`. The table was the symptom; the missing capability was the bug. `useExtensionIcon` is the one entry point now, and it also closes a wart the consolidation made obvious: the asset loader was gated on whether the Lucide lookup succeeded, but Lucide is a lazy chunk, so on first paint a perfectly good `lucide:Camera` was fetched as a file path and logged as a failure on every launch. `ext-icon-data-url-verify` asserts both halves, including that no surface contains a `"tedi.<id>":` key, which is what keeps the table out. See [icon.ts](src/modules/extensions/icon.ts), [ext-icon-data-url-verify.ts](scripts/ui/ext-icon-data-url-verify.ts).
- **The two local sockets, and the two process captures, each become one.** The PTY daemon and the MCP bridge each derived a per-user, per-profile socket name, a path or pipe name, an address and a crash-surviving bind; the daemon had `fnv1a` as a named function while the bridge had the same eight lines and the same two magic constants inlined, under a comment saying it was "matching the PTY daemon's scheme". Separately, `shell_run_command` and `fmt_run_external` each carried ninety lines of drain threads, stop flag, timeout poll and grace window, differing only in a byte cap - concurrency written twice, where every fix had to be remembered in both places. Now `local_socket.rs` and `shell/capture.rs`, the second with tests that actually spawn a process, cap a stream and fail a missing binary. See [local_socket.rs](src-tauri/src/modules/local_socket.rs), [capture.rs](src-tauri/src/modules/shell/capture.rs).
- **CI runs the 76 invariant checks.** It ran typecheck, the import guard, the build and the Rust jobs, but never `pnpm verify` - about fifteen seconds - so every architectural invariant they encode was hand-run only. That is how the bundle defect below reached a release. Two checks first had to stop crashing on a checkout with no `extensions/*`, which is what CI has and what any new contributor has. The import guard also widened from `src/modules/` to all of `src/`, and immediately found six real violations it had been structurally unable to see. See [ci.yml](.github/workflows/ci.yml), [check-imports.mjs](scripts/check-imports.mjs).
- **Two interfaces that were written out twice are derived instead.** `chatStore`'s `Live` restated fifteen of `ToolContext`'s signatures verbatim with its own docstrings on each, and `useTerminalSession`'s options restated all nine callbacks `sessionState` already declares. Both are now derived from the one declaration, so a signature cannot change on one side alone - the same failure the shared MCP tool table exists to prevent. Three type imports in each file fell away, which is the proof the duplication was self-contained. See [chatStore.ts](src/modules/ai/store/chatStore.ts), [useTerminalSession.ts](src/modules/terminal/lib/useTerminalSession.ts).
- **One path vocabulary.** `lib/path.ts` says at the top that it exists so a divergent `basename` copy cannot become a correctness or security risk; there were two more, one of which returned an EMPTY STRING for a path ending in a separator. Plus a fourth `dirname` under another name, a third that handled `/` only, and a private `normalize` that was `toForwardSlash`. Two same-named helpers that meant different things were renamed rather than merged, because the difference was the point. See [path.ts](src/lib/path.ts).
- **`components/ui` and `components/ai-elements` are documented as owned, not generated.** Three docs said to regenerate rather than hand-edit them. In fact `ui/button` and `ui/toast` carry TEDI tokens, six of eleven `ai-elements/` files carry TEDI code, and four `ui/` components are TEDI's own - so following the documented rule would have destroyed all of it. Add a NEW component with the CLI; never re-run it over an existing one.

### Fixed

- **The installed MCP server could not start: the bundle shipped three of the five files it imports.** `server.mjs` imports `./transport.mjs`, which imports `./socket.mjs`, and neither was in `bundle.resources`. These are plain ES modules with no bundler behind them - the app spawns `server.mjs` out of the resource directory and node resolves its siblings on disk - so an installed build died with ERR_MODULE_NOT_FOUND before answering a single request. Invisible in dev, where node resolves against the repo and all five sit together. `surface-verify` now walks the import closure itself and asserts every file it reaches is a bundle resource. See [tauri.conf.json](src-tauri/tauri.conf.json), [surface-verify.ts](scripts/mcp/surface-verify.ts).
- **On Linux, Additional PATH did nothing and `tedi theme` wrote a file the GUI never reads.** `tauri-plugin-store` resolves a bare filename against the app CONFIG dir, which on Windows and macOS is the same folder as the data dir but on Linux is not. Three handle-free readers each located `tedi-settings.json` their own way, and only one of them checked both roots. A file that is not there reads as a setting that is unset, not as an error anyone sees, which is why this was silent. No behaviour change on Windows or macOS. See [ids.rs](src-tauri/src/modules/ids.rs).
- **`pnpm mcp sweep` preflighted into a folder that exists on one machine.** Its `PROJECT` default was an absolute path from the author's own disk, so on anyone else's the preflight `cd` went nowhere and every later check failed for that one reason. It resolves this repo from the script's own location now. See [sweep.mjs](scripts/mcp/sweep.mjs).
- **Documentation that pointed at things which no longer exist.** `components.json` configured the shadcn CLI to inject into `src/App.css`, a file that is not in the repo; the real one is `src/styles/globals.css`, and TEDI.md and ARCHITECTURE.md named it too. `CONTRIBUTING.md` listed a `browser` module that became an extension and omitted two real ones. Seven comments named symbols that had been renamed or deleted, including two functions that left with the browser. Found by checking every doc path against the tree and every backticked symbol in a comment against the code.

### Removed

- **Panel surfaces that rendered nowhere.** `surface` accepted `sidebar-bottom` and `statusbar-right`, which nothing reads. A panel declaring one validated, installed, and then appeared nowhere with no error to go on. Rejecting them says so at `tedi ext validate` instead. All twelve bundled manifests still validate. **Breaking for any extension declaring one, which by definition was not working.** See [manifest.ts](src/modules/extensions/manifest.ts).
- **The "Show SQL Explorer" row in General.** It hard-coded `tedi.sql-explorer` and called `setEnabled` - a second control for what Settings -> Extensions already does, offered to exactly one privileged extension. See [GeneralSection.tsx](src/settings/sections/GeneralSection.tsx).
- **A dead `esbuild` block in the Vite config.** Vite 8 transforms with Oxc and deprecated that key, so the `drop` / `pure` lists that used to strip `debugger` and `console.debug|info|trace` from release builds were ignored from the moment of the bump - the shipped bundle still carried every one of them. Oxc exposes no equivalent, so the dead config is gone rather than left implying a guarantee that had stopped holding; `import.meta.env.DEV` is the gate that really works. See [vite.config.ts](vite.config.ts).

## [0.4.44] - 07-09-2026

### Added

- **An extension can put its own folder on the terminal PATH, and switch off what would shadow it.** A runtime manager is only useful if `php` in a terminal resolves to the version it selected, and the one setting that decides this was unreachable: `ctx.settings` namespaces every key to `ext:<id>:`, so the extension had to ask the user to paste a path into Settings and hope. `ctx.terminal` (permission `terminal:path`) is the sanctioned route - `registerPath` puts a folder first and switches OFF any entry that provides a competing tool, `unregisterPath` puts back exactly what registering changed. Conflicts are PROBED with the same check the Settings rows already ran, so "conflict" means the same thing in both places and a folder that cannot be read is left alone. Nothing is ever deleted: your folders stay listed and switched off, the row says which extension did it, and flipping the switch by hand takes that right away for good. See [host.ts](src/modules/extensions/host.ts), [permissions.ts](src/modules/extensions/permissions.ts), [store.ts](src/modules/settings/store.ts).
- **Additional PATH rows have an on/off switch.** The data model and the PTY layer had honoured `enabled` all along - a disabled entry was already skipped at spawn - and only the UI was missing, offering delete as the only way to deactivate a folder. The switch is the interface catching up with what the backend already did, and it is what lets two folders holding the same `php` coexist. The summary counts what is actually active rather than what is listed. See [AdditionalPathEditor.tsx](src/settings/sections/components/AdditionalPathEditor.tsx).
- **`ctx.ui.pickFolder`, so an extension stops asking anyone to type an absolute path.** The same native picker the folder-tree bridge already opened, so it grants no reach an extension lacked. It exists because a typed path is how a stray character ends up in a Windows root: one arrived carrying a tab, stored fine, and then failed every write with `os error 123`. See [host.ts](src/modules/extensions/host.ts).
- **Find and replace in `ctx.ui.codeEditor`, and an `ini` language.** The editor deliberately shipped without search - and no extension could add it, because the handle hands back no `EditorView`, so there is no way to set a selection or scroll to a match, and CodeMirror only renders the visible viewport so a hit a thousand lines down has no DOM node to fake one on. The only alternative was a `<textarea>`, trading the gutter and the highlighting for `setSelectionRange`. The bar is the editor pane's own `EditorFindReplace`, not CodeMirror's built-in panel, so there is one find bar in the app rather than two that disagree about their layout. See [codeEditor.ts](src/modules/extensions/codeEditor.ts), [App.tsx](src/app/App.tsx).
- **One form vocabulary, enforced instead of noticed.** The caption-over-control pair was written out three times - the SSH dialog, the SCM form controls, the settings label - each with its own copy of the class string and each free to drift. Every dialog and settings row stacks through one `Field` now. Number inputs get the app's own stepper: the native spin buttons only appear on hover, size themselves to the browser rather than the field, and draw a square grey column that fought the shape every other control has, so a number row never matched the string row beside it. See [field.tsx](src/components/ui/field.tsx), [input.tsx](src/components/ui/input.tsx), [form-style-consistency-verify.ts](scripts/ui/form-style-consistency-verify.ts).

### Changed

- **The status bar's three zones are named for the question they answer.** They were "what this is costing you / what is going on elsewhere / what you can press", which is a taxonomy you have to agree with before you can use it. They are **kept**, **folding** and **AI**: what you want to keep seeing when the bar folds, what you are happy to lose, and TEDI's own agent, which is not yours to move. Membership in the locked zone is declared by `defaultZone` alone - a second flag saying "locked" would be a second thing to keep true. See [layout.ts](src/modules/statusbar/layout.ts), [StatusBar.tsx](src/modules/statusbar/StatusBar.tsx).
- **The browser's address bar wears the host's input chrome.** It was filled with `--muted`, the token behind hovered menu rows rather than fields, so the one input in the toolbar read a shade off every other field in the app; focus swapped its background to the pane colour, so a focused field dissolved into the pane instead of staying a field with a ring. See [tedi.browser](https://github.com/IlhamriSKY/tedi.browser).

### Fixed

- **A keyboard shortcut the app claims could not reach an extension's editor.** `useGlobalShortcuts` listens on `window` in the capture phase and stops any match dead, and `search.focus` owns Mod+F app-wide, so the search added above was reachable by no key at all. An extension editor now declares itself and the guard stands down for it while it holds focus - the same shape as the existing exemptions for vim and for a focused terminal. See [App.tsx](src/app/App.tsx), [codeEditor.ts](src/modules/extensions/codeEditor.ts).
...
- **A browser is an extension now, and it is a real Chromium.** TEDI renders its own UI in the webview the OS provides, which is Chromium only on Windows - so a browser built on that webview could offer the DevTools Protocol and Chrome extensions on one platform out of three, and nowhere else. [tedi.browser](https://github.com/IlhamriSKY/tedi.browser) drives a real Chromium instead: it reuses the Chrome, Edge, Brave or Chromium already installed and fetches Chrome for Testing only when the machine has none, so the TEDI download is unchanged and nobody who never opens a browser pane pays for one. The pane is a hole rather than a picture of one: the Chromium window is placed on the pane's own rectangle and held there, so what you see is the browser itself, composited by the GPU - full resolution, the real pointer over a link, native selection and popups, and an ordinary Chrome user agent, which is what lets a Google sign-in through at all. A pane that is not on screen parks its window off-screen rather than leaving it over whatever is now on top, and the process stops entirely after an idle timeout.
- **One pane is one page.** Splitting TEDI is how you open a second, which is also why split, float and canvas work on browsers without the extension knowing they exist - and it puts the page's title and favicon in the pane header, where every other pane already shows what it is holding.
- **On the canvas the browser is a frame stream, not a window.** The canvas workspace view puts every pane on one transformed layer where panes overlap, the whole surface zooms and anything off screen is culled. An OS window can do none of those, so a browser there sat at the right rectangle showing a 1:1 crop of a page you had asked to see at 40%, on top of every window stacked above it. The pane now paints a `Page.startScreencast` stream into a `<canvas>` there and parks the window: an ordinary element, which the transform scales, DOM order stacks and `display:none` culls, with no hole-punching and nothing to keep in sync. It is the same Chrome, the same profile and the same tab either way - only the painter changes - so a signed-in session, an installed extension and an open page all survive switching views. See [screencast.js](https://github.com/IlhamriSKY/tedi.browser), [ui.js](https://github.com/IlhamriSKY/tedi.browser).
- **A floated browser pane can be typed into.** It gets the same frame stream, for a different reason: an adopted window is sent clicks by position but never takes the keyboard on its own, and the host forwards focus to it from a window proc installed on the main window only, so a floated pane drew and scrolled and clicked and swallowed every keystroke.
- **TEDI draws the chrome, and a pane looks the same in every view.** Chromium now runs `--app=`, so its windows have no tab strip, no omnibox and no menu of their own, and the pane's own toolbar - back, forward, reload, address - is the only chrome on either surface. That is what makes the two match at all: a screencast captures the page viewport and cannot capture Chrome's toolbar, so for as long as Chrome drew its own the canvas could never look like the tabs view. It also deletes the omnibox caret bug outright rather than working around it, since there is no omnibox left to put a caret in, and stops Chrome's own Ctrl+T/Ctrl+W reaching into a pane. The price is stated in the extension's README: no extensions button, no profile menu, and no route to the Chrome Web Store from inside a pane - unpacked extensions in `~/.tedi/browser/extensions/` are now the only way in. A Google sign-in is unaffected; none of it depended on Chrome's UI.
- **The frame stream renders at the resolution it is actually displayed at.** It used to size its bitmap from the pane's LAYOUT box, and the canvas view scales its whole window layer with a CSS `transform` - which layout never sees, so `clientWidth` is identical at 40% and at 200% zoom. The page was rendered once at layout size and then stretched by the compositor, which is an upscale, which is why the canvas looked soft next to a real window. The scale now comes from `getBoundingClientRect()` over `clientWidth` times the display ratio, capped at 3x, and a quarter-second watcher catches a zoom because no observer fires for a transform. JPEG quality went 70 to 90 in the same pass: 70 was picked when this was the only surface and nobody had a sharp one to compare it against.
- **The pane's toolbar uses the app's own icons, and reload spins.** The glyphs are the exact `arrow-left`, `arrow-right` and `refresh-cw` path data from the `lucide-react` build the app is compiled against, inlined as SVG rather than fetched through `ctx.ui.icon()` - that helper mounts a React root per call which the host only unmounts on deactivate, so three icons per pane would leak three roots every time one is opened. The reload glyph turns while the page loads, driven by `Page.frameStartedLoading`/`frameStoppedLoading` with a 20s ceiling so a torn-down frame cannot leave it spinning, and it respects `prefers-reduced-motion`.
- **The blank page is a `data:` URL, not `about:blank`.** Measured on Chrome 152: `--app=` is honoured only for http(s) and `data:`, and silently ignored otherwise - `--app=about:blank` and `--app=chrome://version` both open an ORDINARY window reporting 95px of browser UI, while `--app=data:text/html,...` and `--app=https://...` report 39px, the thin frame and nothing else. Cross-origin navigation keeps it: no origin bar appears.
- **A released window is released hidden.** Handing a docked window back used to show it, and switching the workspace view remounts the panel, so every switch between tabs and canvas put a framed Chrome on screen for the length of the handover. `dock_release_window` takes a `visible` flag, and the outside-placement path parks on the way out for the same reason. See [dock.rs](src-tauri/src/modules/dock.rs).
- **Chrome Web Store extensions work, on all three platforms.** The profile is a real Chromium profile, so an ad blocker persists in it. An unpacked folder dropped in `~/.tedi/browser/extensions/` is passed as `--load-extension`, which is the route that needs no window and no clicks.
- **A pane can change its icon at runtime, not only its title.** `ctx.tabs.setExtensionTabState` accepted a `title`; it accepts an `icon` too, taking the same refs `contributes.panels[].icon` does, `data:` URLs included. That is how a browser pane wears the favicon of the page it is showing. See [tabsBridge.ts](src/modules/extensions/tabsBridge.ts), [panes.ts](src/modules/terminal/lib/panes.ts).
...
- **The status bar's right side is three zones, and you arrange them.** Where an item sat was decided by which component happened to render it, so "put the memory meter next to the AI usage meters" was a code change. The bar is now one flat list of individually placeable items over three zones - what this is costing you, what is going on elsewhere, and what you can press - and every item can be dragged into any of them. The arrangement is saved. Dragging shows the real thing: the item lifts under the cursor and the bar itself opens the slot it would land in, computed by the same function the drop persists, so the preview and the result cannot disagree. Folding the bar keeps the first zone whole plus TEDI's own AI wherever it was dragged to, because a folded bar that hides a pending approval hides the one thing asking for attention. See [layout.ts](src/modules/statusbar/layout.ts), [StatusBar.tsx](src/modules/statusbar/StatusBar.tsx).
- **A process monitor, as an extension.** [tedi.process-monitor](https://github.com/IlhamriSKY/tedi.process-monitor) shows every process TEDI is responsible for - the app, its webviews, the PTY daemon that outlives the window, every shell, and any AI CLI attached to one - as a tree with its own CPU and memory. It samples with one long-lived reader rather than spawning a new one per tick, because on Windows a freshly spawned reader costs ~600ms before it has read anything, and it holds ~6% of one core. Hovering the status bar meter draws the last three minutes of CPU and memory as a pixel chart; clicking opens the tree.
- **Save As.** Ctrl/Cmd+Shift+S writes the buffer to a path picked in the native dialog and retargets the pane onto it, so the tab title, the language mode and the next Ctrl+S all follow the file you just saved. Local files only: the dialog picks a local path, so a remote file has nowhere to put one. See [useDocument.ts](src/modules/editor/lib/useDocument.ts), [shortcutHandlers.ts](src/app/lib/shortcutHandlers.ts).
- **One vocabulary for "something is happening", and it is pixels.** TEDI already had one way to draw a small quantity: 4px square cells with a 2px gap, which the status-bar meters, the memory chart and the extension tooltips all use. The AI panel answered with a bouncing dot trio and the agent pill with a spinning circle, in the one place the pixel grid is most visible. Both are the same grid now, animated instead of measured, with the light sweeping diagonally across it. See [pixel-activity.tsx](src/components/ui/pixel-activity.tsx).
- **Max reasoning effort is one ink everywhere it appears.** The level's foil is a gradient clipped to the word, and an SVG stroke cannot hold one that way, so the brain icon beside it wore a single hue drifting through the same palette: at any instant the icon was one colour and the label was a prism, which is exactly the mismatch it was supposed to hide. The icon's stroke now references the gradient itself, and the activity pixels take the same palette while a turn runs at max, so the picker, the icon and the indicator all say max in the same ink. See [globals.css](src/styles/globals.css), [ReasoningDropdown.tsx](src/modules/ai/components/ReasoningDropdown.tsx).
...
- **Font ligatures are off by default.** A fused glyph paints outside its own cell and the editor's WebView repaints only the cell you just typed, so a freshly typed `=>` or `====` could look half erased until the line was redrawn. The setting is unchanged and still one click away. See [extensions.ts](src/modules/editor/lib/extensions.ts).
- **A split is a gutter, not a divider.** The resize handle paints nothing at all now: it simply is the 6px gap between two panes, the same 6px the extension panels inset their cards by, so every seam in the app is one width. The library still inflates the hit region around it and swaps in a resize cursor, and the element stays real because arrow-key resizing is bound per separator and that is the only keyboard route to a pane size. The `resizeHandle` theme colour is gone with it, since there is nothing left to paint. See [resizable.tsx](src/components/ui/resizable.tsx).
- **The AI panel's icons are drawn at the app's own weights.** Thirty-nine glyphs there carried a 1.75 stroke at 11-14px while the rest of the app draws that size at 2, which is why the same Undo2 looked lighter in the AI panel than in Source Control. Measured against the whole app rather than guessed: at 14px and below it draws 2, at 15px and up it draws 1.75, and the AI module now follows both halves of that.
- **The browser puts nothing in the status bar.** It published a status item that duplicated what its own pane header already says. See [tedi.browser](https://github.com/IlhamriSKY/tedi.browser).
- **Linux decides the DMA-BUF renderer for itself.** WebKitGTK's DMA-BUF path aborts the web process with `EGL_BAD_PARAMETER` before the window opens when there is no working EGL display, which is every session with no DRM render node and every non-Mesa desktop. TEDI now turns it off unless the session is Mesa-backed GNOME, KDE or COSMIC, prints the reason it chose on stderr, and still honours an explicit `WEBKIT_DISABLE_DMABUF_RENDERER=1` or `=0`. See [README.md](README.md).
- **CI compiles the Windows and macOS branches on every push.** The Rust job runs on Linux only, so a `cfg`-gated compile error on either of the other two targets was first built by the release matrix, i.e. at tag time. A compile-only job now covers both, so it fails the pull request that introduced it instead. See [ci.yml](.github/workflows/ci.yml).

### Fixed

- **An extension publishing its title rewrote the whole workspaces file, once a second.** `updateExtensionPanelLeaf` returns the same node by reference when its patch changes nothing, which is the invariant every walk in [panes.ts](src/modules/terminal/lib/panes.ts) keeps and which `useAuxTabs` compares against to decide whether anything moved - but its SPLIT arm rebuilt `{...n, children}` unconditionally, so any tab holding more than one pane answered with a fresh root every time. A single-pane tab hid it. `setExtensionTabState` then compounded it: `Array.map` allocates a new array even when every element comes back unchanged, and a new `tabs` identity re-renders the strip and re-runs the workspace snapshot. It cost nothing while the only caller was SQL Explorer changing connection state; a browser pane publishing its title and favicon at 1 Hz turned it into a full serialize and disk write every second, carrying a base64 favicon. Both now bail on a no-op, and [ext-panel-leaf-identity-verify.ts](scripts/panes/ext-panel-leaf-identity-verify.ts) covers the split case a naive test misses. See [useAuxTabs.ts](src/modules/tabs/lib/useAuxTabs.ts).
- **A pane could never drop a favicon it had already shown.** `undefined` meant "unchanged" to the host and the extension only ever sent a truthy icon, so navigating from a site with a favicon to one without left the previous site's mark next to the new site's name, and it survived a restart because the leaf persists the icon. An empty string now means "this page has none".
- **Scrolling a browser pane on the canvas went the wrong way.** The wheel handler negated `deltaX`/`deltaY` before dispatching them. `Input.dispatchMouseEvent` uses the same sign convention as a DOM wheel event - positive `deltaY` scrolls down, which is why every CDP client passes a negative delta to scroll up - so the page moved against the user's own wheel and against whatever scroll direction they had configured. The legacy `WebMouseWheelEvent` convention is the opposite, which is where the confusion comes from, but that is not this API.
- **Chrome's own title bar is cut out of the pane, because no window style removes it.** An `--app` window has no tab strip and no omnibox, but it still paints a bar carrying the page title and a minimise, maximise and close button - and it paints it INSIDE its client area, not as the OS caption. Measured on a real app window: stripping `WS_CAPTION`, `WS_THICKFRAME`, `WS_SYSMENU` and both size boxes moves `outerHeight - innerHeight` from 39px to 37px. Two pixels. So the adopted window is now deliberately made bigger than the pane by the frame and shifted up and left, which puts the PAGE exactly on the pane, and the frame is cut away with the same window region the occlusion clip already uses. The inset is measured rather than guessed - `screenX`/`screenY` give the page's position and `Browser.getWindowForTarget` gives the window's, so the difference is exact - and a window whose frame cannot be measured is handed back instead of shown, which sends the pane to the frame stream that has no frame at all. Both views now show the page and TEDI's own four controls, and nothing else.
- **A browser pane no longer flashes a framed Chrome window before it settles.** On Windows the pane adopts the window as a child, and when the first attempt does not take it used to PLACE the window on the pane meanwhile - a top-level Chrome with its own title bar and window buttons, sitting there through ten retries before being replaced by a frameless child. The window now stays parked while adoption is retried, so the pane is briefly empty instead of briefly wearing another application's chrome, and only falls back to placing once the retries are genuinely exhausted.
- **A pane's runtime icon never actually rendered.** `setExtensionTabState({ icon })`, the field threaded through the tabs bridge, and the leaf serialisation that carries it across a restart were all in place, and the last hop was missing: `LeafIcon` and `EntryIcon` resolve an icon ref through `resolveExtIcon`, which answers `null` for anything that is not `lucide:`/`hugeicon:` so that "the caller's asset loader handles it" - and neither of them had one. Every browser pane wore the generic database glyph instead of the site's favicon while three documents said otherwise. Both call sites now render an image ref, and [ext-icon-data-url-verify.ts](scripts/ui/ext-icon-data-url-verify.ts) guards the shape, because `resolveExtIcon(ref) ?? Fallback` does not look wrong. See [LeafIcon.tsx](src/components/LeafIcon.tsx), [EntryIcon.tsx](src/modules/tabs/components/EntryIcon.tsx).
...
- **Switching an extension on could silently revoke every permission it had.** `ext_toggle` created the extension's state entry with an empty grant list. An extension with no entry runs on its manifest permissions - that is the same fallback `ext_list` reports it with - so the first toggle handed it an entry saying "nothing granted", and because the entry then existed the manifest fallback stopped applying and never came back. Entry-less is the normal state for anything that did not come through the install pipeline, a dev-linked extension included, so this was the difference between an extension that works and one that is enabled, loads, and is refused everything it declares, leaving its pane with no renderer and its tab blank after a restart. The grant is seeded from the manifest now. See [commands.rs](src-tauri/src/modules/extensions/commands.rs).
- **An extension pane said nothing while its extension was still starting.** It is a real state - `bootAll` activates in parallel and a slow member can take seconds - and the pane rendered as though the renderer would never arrive. It now says it is loading while waiting is still the explanation, and reports what is actually wrong once it is not. Disabled and uninstalled are read straight off the store and reported at once, because a timer is the wrong tool for a fact already known. See [ExtensionPanelMount.tsx](src/modules/extensions/components/ExtensionPanelMount.tsx).
- **A Linux box with no `$SHELL` got a terminal that refused to open.** The Unix path fell back to `/bin/zsh`, which is macOS's default and always there, and on Linux usually is not installed at all - and a missing binary fails `CommandBuilder` outright rather than degrading. `$SHELL` is unset in containers, under cron and in some minimal login setups, so the fallback is now the one shell POSIX guarantees. See [shell_init.rs](src-tauri/src/modules/pty/shell_init.rs).
- **The PTY daemon died with the AppImage that started it.** The daemon is built to outlive the GUI, and under an AppImage `current_exe()` points inside the squashfs mount the runtime tears down the moment the GUI exits - so the daemon was left running off a dead mount and took SIGBUS on the first cold page it faulted in, killing every terminal it held. It re-launches the `.AppImage` itself now, which gives it its own mount held for exactly as long as it runs. See [spawn.rs](src-tauri/src/modules/pty_daemon/spawn.rs).
...
- **Everything that controls the app is one MCP surface now; reading and editing files stay native tool calls.** Terminal, pane and browser control existed twice over - once as native AI tools, once as MCP tools reaching the same functions - so the model was handed two vocabularies for one capability and paid the fixed cost of both in every request. The native half is deleted, and `sh`, `read`, `state`, `pane`, `focus_pane`, `wait_for_terminal` and `browser` are served by TEDI's own in-process MCP server out of the SAME table the stdio server publishes to Claude Code, so a tool cannot drift between the two transports. Files stay native because they are not app control: an edit is not a thing the window does. The fixed tool payload drops 38%, 34,838 characters to 21,519. See [tools.mjs](scripts/mcp/tools.mjs), [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts).
- **`sh` can capture what a command actually printed, instead of scraping it off the screen.** The scrollback is a ring of RENDERED rows: it is as wide as the pane, as deep as the ring, and it holds the prompt and the echo along with the output. Reading a remote file through it returned a truncated, re-wrapped copy - a 9,341-byte file came back as 5,807 characters. `capture: true` runs the command on its own channel and returns its bytes: locally through the shell, and on an SSH pane through a russh exec channel that never touches the visible terminal, bounded at 4 MiB of stdout and 15 seconds. The pane is left alone, so nothing races the user's own typing. See [ssh/mod.rs](src-tauri/src/modules/ssh/mod.rs), [useSshLeafState.ts](src/app/hooks/useSshLeafState.ts).
- **One `browser` tool covering the whole pane, where there were twelve.** Seventeen actions - open, navigate, read, console, screenshot, click, type, key, scroll, hover, back, forward, reload, list, close - behind a single definition, so the pane's capabilities cost one schema in the request rather than twelve. Ten of them are reads and run without an approval card; anything that navigates or writes still asks, and `open`/`navigate` skip the card only for a host already trusted this session.
- **Reasoning effort is colour-coded, and `max` is a foil.** Each level from `low` to `max` has its own colour on both the label and the brain icon, so the selected level is legible without reading it. `max` renders as holographic card foil: a slow specular pool crossing a prismatic film, always running, easing in and out so it drifts like liquid rather than sweeping at a constant rate. It is `background-clip: text` over three layered gradients, no canvas and no JS. See [ReasoningDropdown.tsx](src/modules/ai/components/ReasoningDropdown.tsx), [globals.css](src/styles/globals.css).
- **A canvas window can be renamed and pinned from its own right-click menu.** Rename is inline and writes the same `customTitle` the tab strip's rename does, so the name follows the leaf everywhere. Pin lifts a window above every unpinned one and stays there through any amount of click-to-front; pinned windows still stack among THEMSELVES by the same last-clicked rule, because the pin rides the existing `z` rather than replacing it. See [CanvasView.tsx](src/modules/panes/CanvasView.tsx), [panes.ts](src/modules/terminal/lib/panes.ts).

### Changed

- **The tools picker groups in two levels.** One flat row per server put API Client and SQL Explorer under a single "Extensions" line, which is not what they are - they are separate capability sets that happen to arrive the same way. Sections open by default and groups inside them stay closed, so the shape of what is switched on is visible without scrolling past every tool. See [ToolsPicker.tsx](src/modules/ai/components/ToolsPicker.tsx), [catalog.ts](src/modules/ai/tools/catalog.ts).
- **Installed MCP servers are enumerated when the panel mounts, not when the popover opens.** Discovery ran on the first open, so the count read 38/38 until the walk finished and only then became 80/80 - the picker was showing a number it knew was incomplete. It now runs in the background at mount, and `tedi` and any installed server (chrome-devtools among them) are listed by `/mcp` when active.
- **Tool calls in the transcript are named after what they are.** A generic title-caser turned `mcp__tedi__ssh` into "Mcp Tedi Ssh"; acronyms are now kept, so it reads "MCP TEDI SSH". Twenty entries for tools that no longer exist were dropped along with them. See [tool.tsx](src/components/ai-elements/tool.tsx).
- **`none` is gone from the reasoning menu.** It is not a level anyone picks on purpose - it turns reasoning off - and it is still sent where it is a model's own documented default, so nothing changes in what goes out on the wire.
- **The MCP dialog counts what is on instead of estimating tokens.** A figure like "~3.2k tokens per request" looks measured and is a guess; what the dialog actually chooses is which capabilities to expose. The real per-request cost is printed by `tool-budget-verify.ts`, which serializes the true schemas.
- **The canvas zooms out to 15% and the surface has an edge.** At 25% a hard flick could throw a window far enough out that finding it again meant Fit-all and a long drag; the space is now bounded, so a window cannot leave the reachable area.
- **Terminals sit at the same inset in every view.** The pane tree took a `flush` prop that removed the padding in some layouts and not others, so a terminal moved by a few pixels depending on where it was rendered. See [PaneTreeView.tsx](src/modules/panes/PaneTreeView.tsx).

### Fixed

- **An SSH pane could open a terminal and then never run a command in it.** The tools resolved a pane through the tab tree, and the handle for a just-opened pane is registered a frame later, so the call that followed `open` found nothing to write to - the terminal appeared, the command went nowhere, and the model was left describing a shell it could not reach. Resolution now waits for the handle instead of reading a tree that has not caught up.
- **A terminal read returned text that had been re-wrapped by the pane.** xterm stores a wrapped line as several rows and marks the continuations; the reader joined them with newlines, so any line longer than the pane came back broken at the pane width, mid-word. Continuation rows are rejoined at the source, which fixes every reader at once. See [useTerminalSession.ts](src/modules/terminal/lib/useTerminalSession.ts), [terminal-buffer-wrap-verify.ts](scripts/terminal/terminal-buffer-wrap-verify.ts).
- **The whole CLI MCP surface died the moment a browser pane was open.** The bridge resolved the window with `get_webview_window("main")`, which in Tauri v2 matches only a window holding exactly ONE webview - and a browser pane adds a second. Every command that went through the bridge then failed with a window that plainly existed on screen. Six call sites now ask for the window itself. See [mcp_bridge.rs](src-tauri/src/modules/mcp_bridge.rs).
- **The browser page painted outside its own pane.** A native child webview is positioned by the OS and knows nothing about CSS layout, so it was placed at the pane's rect even when an ancestor had already clipped that rect - the page spilled over neighbouring panes and past the edge of a scrolled container. The visible box is now computed by walking the ancestors' clips; on Windows the difference is cut out of the window region, elsewhere the webview hides rather than draw somewhere wrong. See [BrowserPane.tsx](src/modules/browser/BrowserPane.tsx).
- **On the canvas, the page drew over the windows stacked above it.** Same root cause seen from the other side: a webview composites above the entire DOM, so raising another window over a browser pane raised its header and body and left the page in front. Panes that overlap and paint above are now cut out of the region too, which costs a few rect reads and changes nothing in tabs or splits, where panes never overlap. See [overlaySuppress.ts](src/modules/browser/lib/overlaySuppress.ts).
- **Opening a menu over a browser pane undid the fix above.** The overlay branch assigned the hole list rather than appending to it, so the pane-occlusion holes vanished for as long as any dropdown was open. Both sets are collected now.
- **Browser back, forward and reload did nothing.** They were dispatched at the driver rather than at the pane, so with a leaf id in hand they addressed no page at all; they go through the per-leaf dispatch the other actions use.
- **Eleven MCP driver methods were Windows-only by accident.** Anything absent from the bridge table falls through to CDP, which only runs on Windows - so newly added methods silently became platform-specific. The verify now also checks the reverse direction, that every in-realm method is routed through the socket, and it caught two the first time it ran. See [driver-verify.ts](scripts/mcp/driver-verify.ts).
- **A just-opened local shell could be adopted into a second pane.** The daemon can list a session in the ~200 ms before `pty_open` returns, and the adopter would then open another leaf for a terminal that already had one. Local sessions are tracked as they open and skipped by the adopter. See [pty-bridge.ts](src/modules/terminal/lib/pty-bridge.ts), [useAdoptDaemonSessions.ts](src/app/hooks/useAdoptDaemonSessions.ts).
- **WebView2's own context menu opened over app chrome.** Right-clicking anything that was not a text field produced the browser's menu - reload, save as, view source - on top of the app's. It is suppressed everywhere except editable fields, which keep cut, copy, paste and the spell-check list. See [main.tsx](src/main.tsx).

### Security

- **The in-app browser refuses `file://` and cloud-metadata addresses outright.** Approval is the wrong guard for these: `file://` reads local files through a surface whose whole purpose is handing page text back to the model, and `169.254.169.254` hands out instance credentials to any plain GET. Neither is something an approval card describes well enough for a person to catch in the moment, so both are rejected before one is drawn. See [security.ts](src/modules/ai/lib/security.ts).
- **Auto-approval is decided per action, not per tool.** A tool that both reads and writes cannot be marked safe as a whole. Each tool now names which of its actions are free, so the read half runs without a card while everything else asks - and `open`/`navigate` ask unless the host was already trusted in this session. See [mcp.ts](src/modules/ai/tools/mcp.ts).

## [0.4.40] - 02-09-2026

### Added

- **Reasoning effort, per model, mapped to the parameter the provider actually has.** A control appears beside the model picker only when the selected model really exposes one, and it offers that model's own values - because there is no universal vocabulary. GPT-5.6 takes `none low medium high xhigh max`, GPT-5.5 the same without `max`, GPT-5.4-mini/nano the same again, and GPT-5.x-codex takes `low medium high xhigh` and rejects `none` outright. Claude takes `low medium high xhigh max` under a different field, and Sonnet 4.6 predates `xhigh`. Gemini 3.x takes `minimal low medium high`. Grok 4.5, the gpt-oss pair and DeepSeek each take a third, fourth and fifth set. The picker names the request field it writes, so what a level does is inspectable rather than folklore. See [reasoning.ts](src/modules/ai/lib/reasoning.ts), [ReasoningDropdown.tsx](src/modules/ai/components/ReasoningDropdown.tsx).
- **The default is still to send nothing.** `Auto` is not a value any API knows; it omits the parameter and lets the provider's own default stand, so the request is byte-identical to before this existed until a level is picked. The menu shows what that default is, which is the only way to see that **GPT-5.4-mini and -nano default to `none` and have been doing no reasoning at all**. A level is stored per `provider::model`, so switching models cannot carry one across, and a value the model does not accept is dropped on the way out rather than sent.
- **Models with no reasoning control get no control.** Claude Haiku 4.5 and Gemini 2.5 Flash are token-BUDGET models, not enum ones - `effort` is a hard error on Haiku - so they are deliberately absent rather than given a knob that lies. Gemma, Llama, the non-reasoning Grok ids, and every arbitrary gateway (SumoPod, AgentRouter, OpenAI-compatible, LM Studio) are absent for the same reason: the model behind a gateway is whatever the endpoint chose to serve, there is no capability probe, and an unsupported parameter is a 400 that kills the turn.
- **A check that proves the level reaches the provider.** Not frontend state: it builds the real client for every supported model, runs a real request through a stub fetch, and reads the level back out of the captured HTTP body - every value of every model, plus `Auto` sending nothing, plus unsupported models never receiving it, plus the merge keeping ChatGPT's mandatory `store: false` and OpenAI's prompt-cache key intact. See [reasoning-effort-verify.ts](scripts/ai/reasoning-effort-verify.ts).
- **The inline-completion path uses the same table.** It used to set `reasoningEffort: "low"` under seven provider namespaces at once plus a Google thinking level - a shotgun, because a model id alone does not say which client built it. It could not know that `low` is not the floor on the GPT-5.x line, that codex rejects `none`, or that Gemini 2.5 takes an integer budget and 400s on a level.

### Changed

- **The browser tools are held back once per turn, not once per step.** Eleven of the twelve need a `leafId` from an already-open pane, so with no browser open they cannot be called; withholding them saves about 2,780 tokens of every request. Deciding that per STEP meant the ordinary flow - open a pane, use it - added eleven tool definitions mid-turn, and adding or removing a tool invalidates Anthropic's tools cache and everything cached behind it, so it rewrote the whole prefix in exactly the flow it was meant to help. It is decided once per turn now, and the same set feeds the system prompt, so the prompt no longer instructs seven tools the request does not carry. See [catalog.ts](src/modules/ai/tools/catalog.ts).
- **Only the static cache breakpoint takes the 1-hour TTL.** A 1h write costs 2x base against 1.25x, so it belongs on the system block, which is byte-stable for a whole session and read by every later request. The newest user message and the rolling tool tail move every turn and every step; a longer TTL there protects a prefix that has already changed while charging the premium every time.
- **MCP tool discovery is bounded in wall-clock, not just in pages.** Following `nextCursor` up to twenty pages at the SDK's 60-second default is a twenty-minute stall on the turn's critical path, where before it was one page. A budget across the whole walk plus a shorter per-page timeout bounds it at seconds. The walk also re-primes the SDK's per-tool metadata from the complete list, because `listTools` clears that cache on every call and would otherwise leave only the last page's output schemas and task flags.
- **The in-process MCP server is no longer keyed on the working directory.** It spawns nothing and reads no cwd, so keying on one only guaranteed a miss: any workspace or terminal switch discarded a live server, rebuilt it and re-listed its tools for a byte-identical answer. It was the one place MCP work repeated per turn.

### Fixed

- **Two MCP tools claimed to be safer than they are.** `screenshot` was annotated read-only while writing a caller-chosen file path, and `run_command` non-destructive while reaching project-wide search-and-replace, pane and terminal closes, and arbitrary extension code. The whole point of those annotations is to let a client auto-approve on them, so both are dropped and fall back to the spec's pessimistic default.
- **An extension pack switched on mid-session stayed invisible after a cold start.** The stdio server is launched at session start, routinely before TEDI is open, so its first tool list answers from a disk snapshot - and caching that on the pack list alone froze the stale answer for the rest of the session. It now retries until a query actually reaches the app.
- **The extension-tool cache was one fixed path shared by every profile on the machine.** A dev profile's cold start served the release profile's tool list and routed calls from it, and on a shared temp directory another local user could pre-plant the file. It is keyed by bundle id now, like the settings file and the bridge socket.
- **`sh` and `wait_for_terminal` rebuilt every terminal's scrollback to watch one.** Moving the reduction in-realm saved the socket bytes but left the per-pane scope behind, so each poll - about seven a second for the life of a command - hashed a 200-row buffer for every open terminal on the UI thread. The poll is scoped to the pane it is actually watching.
- **Two schema bounds refused input the runtime already tolerated.** A sub-agent `depends_on` edge out of range is dropped and reported in the result, and `summary_kb: 0` is normalised - but the bounds rejected the whole call instead, on the stated premise that a schema violation aborts the turn. The installed SDK does the opposite: it returns a correctable tool error and the turn continues.
- **A large MCP result could still dominate the context window.** Third-party text was capped; the base64 image and audio parts beside it were not, and those are the expensive half - forwarded into the message and then replayed with the history for the rest of the session.
- **The composite MCP methods had no guard.** `sh`, `wait_for_terminal` and the prompt wait run on a driver with no DevTools connection at all, which is what makes them work on macOS and Linux; the day one of them reaches for CDP it throws only on those platforms, where nobody developing on Windows would see it. The rule is checked now, rather than asserted in a comment. See [driver-verify.ts](scripts/mcp/driver-verify.ts).
...
- **The shortcut audit now covers all four layers that can claim a keystroke.** `keybindings-collision-verify` checked the catalog against itself and against a focused terminal; it now also checks every installed extension against core and against the other extensions, and asserts that every vim chord core also binds falls through to a focused vim editor. That last check reads the installed `@replit/codemirror-vim` too, so a version bump that adds a chord fails the check rather than silently losing the key. See [keybindings-collision-verify.ts](scripts/ui/keybindings-collision-verify.ts).

### Changed

- **Every code editor in the app now starts at the same place.** `ctx.ui.codeEditor`, the editor TEDI hands to extensions, padded 8px above its first line while the editor pane sits flush, so SQL Explorer and API Client both opened with a gap under their toolbars that no other editor had, under a comment claiming the theme matched the pane. Geometry and typography are now asserted identical between the two themes rather than described as identical. The extension editor also picks up the gutter fix the pane has carried for a while: a solid background above the scroll layer, so horizontally scrolled SQL or a long request URL cannot bleed through the line-number column. See [codeEditor.ts](src/modules/extensions/codeEditor.ts), [editor-style-consistency-verify.ts](scripts/ui/editor-style-consistency-verify.ts).
- **The `+` menu says Agents and Editor.** `Agent...` was singular for a picker that opens as many panes as you ask it for, and `Note` described the file on disk rather than the thing it opens. The item that opens a scratch editor now carries the same icon an editor pane does. See [NewTabMenu.tsx](src/modules/tabs/components/NewTabMenu.tsx).

### Removed

- **The skills installer left its whole backend behind when the feature was removed.** Two Tauri commands (`github_head_sha`, `github_repo_text_files`) and roughly 215 lines under them: a git smart-HTTP `info/refs` parser, a codeload archive downloader, and a zip-bomb-guarded in-memory text extractor, none of it reachable from anywhere in the app, the scripts, or any extension. `mcp_bridge_info` goes the same way, a registered command nothing has ever called, whose docstring claimed two callers that do not exist. Also gone: `hasBridge`, `isSignedInToChatGpt`, and a duplicated base64 frame decoder and MIME helper that each existed twice. The backend command count is 126.
- **Documentation that described a feature the app no longer has.** `TEDI.md`, `ARCHITECTURE.md` and `README.md` still documented skills in four places, still claimed 124 commands and 19 frontend modules (`automation/` and `mcpInstall/` were missing from both the tree and the table), disagreed with themselves about how many files feed the automation bridge, and left `tedi.ai-usage` out of the extension table. `commandRegistry.ts` still described the merge-spread that the capability registry replaced, down to a count that was wrong when it was written.

### Fixed

- **An extension could take a keystroke the shell needed, in every terminal and every SSH session.** The core dispatcher has a whole gate for this: a focused terminal owns every bare-Ctrl control code (`^A` beginning-of-line, `^E` end-of-line, `^R` reverse-search, the tmux and GNU screen prefix) and every bare-Alt readline meta sequence, so the byte reaches xterm instead of firing an app action. The extension dispatcher had no gate at all, and it is a capture-phase `window` listener that calls `preventDefault` the moment it matches, so an extension declaring `Ctrl+A` silently ate that key everywhere, forever. It now applies the same rule. See [useExtensionShortcuts.ts](src/modules/shortcuts/lib/useExtensionShortcuts.ts).
- **An extension shortcut could take over a built-in one, but only after you rebound something.** Both dispatchers listen on `window` in the capture phase, so which one wins a shared chord came down to which registered first. The core hook re-registers its listener every time the shortcuts preference changes, which moved it behind the extension one from that point on: the built-in action worked until the first visit to Settings, then quietly stopped. Core now wins a chord it already claims regardless of order, and the losing extension binding is named once in the console instead of failing in silence. `tedi.beautify` shipped exactly this collision (`Mod+Alt+B`, the same chord as Toggle right panel) and moves to `Mod+Alt+F` in its own 0.2.1. See [shortcuts.ts](src/modules/shortcuts/shortcuts.ts).
- **Vim mode never received nine of the keys vim defines.** `Ctrl+D` and `Ctrl+F` and `Ctrl+B` (half page, page down, page up), `Ctrl+E` (scroll a line), `Ctrl+I` (jump forward), `Ctrl+W` (delete word back), `Ctrl+T` (indent), `Ctrl+P`, and `Ctrl+[` (the ergonomic Escape) were all claimed by the app on Windows and Linux, where `Mod` is Ctrl. Raising CodeMirror's own keymap precedence could never have helped: the app listens on `window` in the capture phase and preventDefaults before CodeMirror sees the key at all. A focused editor with vim mode on now keeps them, scoped to real keyboard focus in an editor that actually has vim (an extension's `ctx.ui.codeEditor` is CodeMirror too, and would have swallowed them into nothing) and to the chords the vim keymap really binds, so `Ctrl+K`, `Ctrl+G` and `Ctrl+H` keep their app action rather than going dead. Nothing changes for a terminal, a non-vim editor, or anyone who leaves vim mode off. See [shortcuts.ts](src/modules/shortcuts/shortcuts.ts), [vim.ts](src/modules/editor/lib/vim.ts), [App.tsx](src/app/App.tsx).
- **Rebinding a shortcut said nothing when the chord was already taken.** Settings let you hand one chord to two actions and find out later that one of them had stopped working, which is the same failure the two bugs above produce. A row whose chord is claimed elsewhere now says so, across built-ins and every installed extension, with your own overrides applied on both sides. See [ShortcutsSection.tsx](src/settings/sections/ShortcutsSection.tsx).
- **The dev-server pill polled through a second, hand-rolled copy of the shared visibility poll.** `useLiveUrl` rebuilt the start/stop, focus, blur and resume-coalescing logic that `useVisibilityPoll` already owns and that the git and file-tree polls already use, including rebuilding its timer on every tab change. It uses the shared helper now, and the check that guards it asserts the property where it actually lives instead of pattern-matching a copy. See [useProjectUrl.ts](src/app/hooks/useProjectUrl.ts), [windowResume.ts](src/lib/windowResume.ts).
...
- **Every MCP tool call killed the server it was sent to.** The transport is a Proxy with a catch-all property handler, and the server reaches it through `await`. `await` decides whether something is a promise by reading `.then` and checking it is callable, so the proxy answered that question with a function and the language adopted it as a thenable: it called `then(resolve, reject)` and waited for an answer that was never going to come. `then` is not a bridged capability, so the call fell through to the DevTools path, found no such method, and threw inside a promise nothing was holding. Both halves of that were fatal. Every tool hung, and the stray rejection took the whole process down, which a connected CLI reported as `Connection closed`. The proxy now refuses `then` and every well-known symbol, which is the general rule: a proxy that answers everything must still say no to the protocol the language itself uses to ask questions about it. See [transport.mjs](scripts/mcp/transport.mjs).
- **The local-socket bridge was unreachable, so the feature it replaced the debug port with never ran.** The handshake file naming the socket and the per-run token was written one directory too deep, because `app_data_dir()` already ends in the bundle id and the path joined it a second time. The client looks beside `tedi-settings.json` and found nothing, so it answered "TEDI is not running" on a running app, forever. Nothing said so: the transport falls back to the DevTools port and only surfaces the bridge's reason when that fails too, so on Windows with the port open it looked like it worked, and on macOS and Linux, where no port is ever opened, the whole surface was simply dead. A test named for this exact property was green on top of it, because it asserted the filename and that the path mentioned the bundle id somewhere, both true of a path one level too deep. It compares the parent now. See [mcp_bridge.rs](src-tauri/src/modules/mcp_bridge.rs).
- **The MCP server could read one TEDI's settings while typing into another one's window.** `TEDI_BUNDLE_ID` picked the settings file and the socket; the DevTools port beside it was a bare environment variable with a hard-coded fallback. With the id set and the port not, which is every hand-written config, the pack switches came from the app you named and the keystrokes went to whichever app happened to hold the usual port. The port is now resolved from the same profile the bundle id names, and a profile with the channel switched off says so instead of reaching for someone else's. See [server.mjs](scripts/mcp/server.mjs), [transport.mjs](scripts/mcp/transport.mjs).
- **Reading a terminal by id could answer with a different pane's scrollback.** `read` and `wait_for_terminal` fell back to the last terminal when the leaf id they were given did not match one, so a question about pane A came back as pane B's output, labelled a success. Worse for the wait, which is a claim about one pane: an agent waiting on its build was told it had finished by an idle shell sitting next to it. `sh` already had this right and documented the rule; all three now agree, and the fallback applies only when no leaf was named. See [server.mjs](scripts/mcp/server.mjs), [driver.mjs](scripts/mcp/driver.mjs).
- **The SSH connect card blinked, and said nothing when a connection failed.** Two causes. Its fade-out left the animation fill mode at the library default, so the card faded away and then snapped back to full opacity for the last frames before it unmounted. And its visibility was derived from the instantaneous status, so the moment a chain gave up the card vanished: on a jump host that fails fast, that reads as the indicator closing and reopening on every retry, and the reason it died was never drawn at all. It is now one card per attempt, raised on the first dial and taken down only once the attempt has actually ended, with a red chain naming the hop that broke and the reason it gave. A disconnect you asked for is still not a failure. See [SshConnectOverlay.tsx](src/modules/ssh/SshConnectOverlay.tsx), [status.ts](src/modules/ssh/status.ts).
- **Closing a tab sent you to the far end of the strip.** Both close paths activated the left neighbour. Opening a file appends a tab and activates it, so its left neighbour is whatever happened to be last in the strip, a tab you never chose and were probably not working in, and the more tabs you had open the further from your place you landed. Closing a tab now goes back to the one that was active before it, the answer every editor gives; the neighbour survives only as the fallback for a restored workspace with no history to go back to. See [useTabs.ts](src/modules/tabs/lib/useTabs.ts), [tabHelpers.ts](src/modules/tabs/lib/tabHelpers.ts).
- **A preference that defaults to `null` skipped every check, including its own allowed set.** The branch was written for "there is no type to infer" and read as "there is nothing to check", so `defaultProviderId`, a closed union of twelve ids where the lookup throws on anything else, accepted any string at all: the write reported success and the next request failed with `Unknown provider`. One such value was found sitting in a real profile. Model ids stay unchecked on purpose, because those really do come from a live catalogue. See [store.ts](src/modules/settings/store.ts).
- **The MCP switch could read as off while Claude Code was still loading the server.** Claude Code takes `mcpServers` from three places in one file, and the switch only ever looked at the top-level one; `claude mcp add` writes to the per-folder section by default. An entry there showed as off and turning it off removed nothing. Every scope is read and cleared now, and installing converges on a single entry. See [install.ts](src/modules/mcpInstall/install.ts).

### Changed

- **The project row in the Install MCP dialog names the folder it means.** It said "This project", which reads as global, while it only ever covered the folder TEDI has open. An AI CLI reads the `.mcp.json` of its own working directory, so a checked-in one in some other repo kept loading TEDI after that switch was turned off, which looked like the switch being ignored. The row now reads `<folder>/.mcp.json` and the full path is on hover. This repo's own `.mcp.json` is untracked and ignored for the same reason: the entry names an absolute path into one machine's install, so a committed copy hands everyone else a path that does not exist and is invisible to the switch that is supposed to control it. See [install.ts](src/modules/mcpInstall/install.ts), [McpInstallButton.tsx](src/modules/mcpInstall/McpInstallButton.tsx).

## [0.4.33] - 27-08-2026

### Security

- **An agent could switch off its own approval gate.** `set_setting` is in the default-on Settings pack, and `approvalMode` was an ordinary preference whose allowed values included `yolo` - the mode in which every tool auto-approves. One approved call disarmed every card for the rest of the session, and the same route could clear `disabledTools` (the AI tool picker's off-list, so every tool you unticked came back) or repoint `openaiCompatibleBaseURL` / `lmstudioBaseURL` / `openaiCompatibleInstances`, which are the endpoints that receive `Authorization: Bearer <key>` and the whole conversation. `terminalEnvPath` is prepended to the PATH of every shell TEDI spawns. Those six keys now refuse the write and say to change them in Settings. The rule already existed for `automationPort` and `mcpDisabledTools`, which were kept out of `Preferences` entirely for exactly this reason; keeping a key out of that object only ever protected that key. See [store.ts](src/modules/settings/store.ts).
- **The pack switches did not switch anything off for the client they exist to constrain.** Turning a pack off filtered `tools/list` and nothing else, so a connected CLI that had already listed once, or simply guessed a name, still reached `eval_js`, `sh` or `set_setting` by calling it. The refusal branch that was supposed to catch this could never run: it sat inside `if (!tool)`, and a disabled tool is still in the table. The check now runs before dispatch, on both transports, and the "unknown tool" message lists only what is actually enabled instead of printing the full set. The switches are also re-read rather than snapshotted at startup, so flipping one lands on the next call instead of after the CLI restarts. See [server.mjs](scripts/mcp/server.mjs), [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts).
- **Private panes were not absent from the automation surface, despite the guarantee.** The pane list, the focused-leaf lookup and the DOM reader all queried `[data-pane-leaf]`, which is stamped on every leaf without a privacy check, so a private pane's id and kind were listed and a private editor's buffer was readable line by line through CodeMirror's rendered text. The browser tool's `url` and `console` actions took a raw tab id straight to Rust, skipping the filtered context its sibling actions used. All four now filter, and the attribute stays in place so file-drop and Ask-AI keep working inside a private pane. See [PaneTreeView.tsx](src/modules/panes/PaneTreeView.tsx), [driver.mjs](scripts/mcp/driver.mjs).
- **The shell denylist never saw the command that actually ran.** An extension holding `shell:transform` rewrites every command the agent runs; the check inspected what the model wrote, then the transformed string was executed. Seven call sites shared the gap, and the comment above them claimed the opposite. Both strings are now vetted in one place, the raw one first so a refusal quotes what the model wrote and the effective one because that is what reaches the shell. See [shell.ts](src/modules/ai/tools/shell.ts).
- **On Windows the resolved-path guards matched nothing.** `std::fs::canonicalize` returns an extended-length path, so `C:\Windows\...` became `//?/c:/windows/...`, and the frontend's prefix tests (`startsWith("c:/windows/")`, the drive-root pattern) failed against it. The literal check caught a direct write, which meant the resolved check only ever ran for a path that arrived through a symlink - the exact case it exists for. The prefix is now stripped, UNC form included. Separately, the scope test itself never followed symlinks, so a link committed into a repo stayed in scope forever and an unattended worker could read and write through it; the refusal paths now canonicalize first. See [fs/mod.rs](src-tauri/src/modules/fs/mod.rs), [context.ts](src/modules/ai/tools/context.ts).
- **Two tools ran with no approval where their siblings ask.** `control_browser` navigated to any host without the egress card that `open_browser` and `navigate_and_read` both raise, and spawning a mutating sub-agent had no card at all even though the worker it starts gets auto-approving edit and shell tools - so the one decision covering everything that worker writes and runs was never put to you. Read-only sub-agents stay automatic. See [terminal.ts](src/modules/ai/tools/terminal.ts), [subagent.ts](src/modules/ai/tools/subagent.ts).

### Added

- **An outside AI CLI now reaches TEDI over an authenticated local socket instead of a debug port.** The old way in was the WebView2 DevTools Protocol, which cost five separate things: it is Windows-only (the flag that opens the port is compiled out everywhere else, while the indicator lit up regardless), a page target accepts exactly one client so a second CLI wedged the first, CDP has no authentication of any kind, the port could only change on a restart because WebView2 fixes its browser arguments before the first webview exists, and nothing in-process could answer, so the whole surface had to be re-expressed as injected JavaScript. The bridge is a named pipe on Windows and a unix socket elsewhere - the same primitive the PTY daemon already uses - with a token minted per app run, so there is no port, no firewall prompt, and the OS decides who may connect before a byte is read. CDP survives only for what genuinely needs it: real keyboard and mouse input and window capture, because a synthetic DOM event is not a trusted one. A session that never calls those opens no DevTools connection at all. See [mcp_bridge.rs](src-tauri/src/modules/mcp_bridge.rs), [socket.mjs](scripts/mcp/socket.mjs), [transport.mjs](scripts/mcp/transport.mjs).
- **TEDI's own agent can finally see the other agents in the window.** An outside CLI could already read what the built-in agent was doing; the reverse was blind, even though everything needed was already tracked. The per-turn environment block now names the AI CLI running in each pane and what it is doing, so "terminal 3 is a shell" became "terminal 3 is claude, waiting on a prompt" - which is the difference between helping and interrupting. It costs about two tokens per pane and only renders when a CLI is actually there. See [terminalSnapshot.ts](src/app/lib/terminalSnapshot.ts), [envContext.ts](src/modules/ai/lib/envContext.ts).
- **The capabilities an agent can reach are a registry now, not an object seven files took turns appending to.** Availability used to depend on mount history, because three of those contributors ran inside React effects, and there was no way to call a capability except through the DevTools socket. See [bridge.ts](src/modules/automation/bridge.ts).

### Changed

- **One tool table, read by both MCP servers.** Names, descriptions and schemas lived in two hand-maintained copies, and the rule that they should not was written down years before it was followed. They had drifted: `ssh` meant `{action, id}` to an outside CLI and `{connectionId}` to the built-in agent, so the documented call silently listed connections instead of opening one - a plausible wrong answer from a correctly formed request. The in-process server moved onto the SDK's low-level server so it serves that shared JSON Schema verbatim; the high-level Zod path was the mechanism of the bug, because its object parse drops unknown keys rather than complaining. The pack categories are derived from the same table instead of a second list of names, and the check that used to compare those two lists now asserts the actual rule: the in-process server may declare no description or schema of its own. Adding a tool is one entry in one file. See [tools.mjs](scripts/mcp/tools.mjs), [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts), [packs.ts](src/modules/mcpInstall/packs.ts).
- **`scripts/director/` is `scripts/mcp/`, and the rest of `scripts/` is grouped by feature.** The name predated the thing being an MCP server, and the directory had grown to about seventy files in one flat list where nothing indicated which subsystem a check belonged to. They now sit under `ai/`, `terminal/`, `ssh/`, `browser/`, `scm/`, `ext/`, `editor/`, `workspace/`, `ui/`, `mcp/` and `release/`; the runner discovers them recursively, so `pnpm verify ssh` still works and `pnpm verify mcp/` selects a folder. `pnpm director` is now `pnpm mcp`. See [scripts/README.md](scripts/README.md).

### Fixed

- **`mcp__tedi__ssh` reported that it had opened a connection while opening nothing.** Building the tool list and executing a tool resolved the in-process server under two different cache keys whenever any pack was switched off, so execution reached a second server built from stub dependencies whose "open a tab" function always returns false - and the handler threw that answer away and reported success regardless. Both call sites now share one dependency object, and the handler reports the refusal. See [mcp.ts](src/modules/ai/tools/mcp.ts).
- **A failing tool looked like a successful one.** Every business failure in the in-process server returned a success envelope whose text happened to contain the word "error", so the host rendered a failed call as completed and the model had to notice the word in the JSON. Failures now set the protocol's error flag, matching the other transport, and results are capped at the same twenty thousand characters instead of being unbounded - `inspect settings` was shipping the entire preference store, custom theme and full terminal palette included. See [tediMcpServer.ts](src/modules/ai/lib/tediMcpServer.ts).
- **Three defects in the MCP client's process handling.** Killing a server held the global process registry across a blocking wait on the child's stdin, which could stall every MCP command in the app behind it; the same lock was held across a fork and exec on Unix. A dying server's wait thread removed whatever was registered under its id, so on the reconnect path it deleted the brand-new entry moments after it was inserted, leaving a running server the app could no longer write to. And nothing disconnected MCP servers on quit, so on macOS and Linux every server TEDI had spawned outlived it. See [mcp.rs](src-tauri/src/modules/mcp.rs), [mcpClient.ts](src/modules/ai/lib/mcpClient.ts).
