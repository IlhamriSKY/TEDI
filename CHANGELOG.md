...
- **Dead code and redundant dependencies were removed.** An unused `useIsMobile` hook, three extension contribution categories that never had a consumer (`slashCommands` / `themes` / `editorThemes`), and three transitively-satisfied direct dependencies (`@uiw/codemirror-themes`, `grep-matcher`, `async-trait`) are gone ([`registries.ts`](src/modules/extensions/registries.ts), [`manifest.ts`](src/modules/extensions/manifest.ts), [`package.json`](package.json), [`Cargo.toml`](src-tauri/Cargo.toml)).
